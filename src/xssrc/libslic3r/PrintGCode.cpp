...
void PrintGCode::output()
{
    ...
    for (auto *region : _print.regions)
    {
        {
            const Flow flow{
                region->flow(frExternalPerimeter, layer_height, false, false, -1, first_object)};
            auto vol_speed =
                flow.mm3_per_mm() * region->config.get_abs_value("external_perimeter_speed");
        ...
        {
            const Flow flow{
                region->flow(frPerimeter, layer_height, false, false, -1, first_object)};
        ...
        {
            const Flow flow{
                region->flow(frSolidInfill, layer_height, false, false, -1, first_object)};
        ...
        {
            const Flow flow{
                region->flow(frTopSolidInfill, layer_height, false, false, -1, first_object)};
            auto vol_speed =
                flow.mm3_per_mm() * region->config.get_abs_value("top_solid_infill_speed");
            if (config.max_volumetric_speed.getFloat() > 0)
                vol_speed = std::min(vol_speed, config.max_volumetric_speed.getFloat());
        ...
        if (_print.has_support_material())
        {
            ...
            auto vol_speed =
                flow.mm3_per_mm() * first_object.config.get_abs_value("support_material_speed");
            if (config.max_volumetric_speed.getFloat() > 0)
                vol_speed = std::min(vol_speed, config.max_volumetric_speed.getFloat());
        ...
        if (_print.config.first_layer_extrusion_width.get_abs_value(layer_height) > 0)
        {
            const Flow flow{
                region->flow(frPerimeter, layer_height, false, false, -1, first_object)};
            //          auto vol_speed = flow.mm3_per_mm() *
            //          _print.config.get_abs_value("first_layer_speed"); if
            //          (config.max_volumetric_speed.getFloat() > 0)
            //              vol_speed = std::min(vol_speed, config.max_volumetric_speed.getFloat());
    ...
    // GCode sets this automatically when change_layer() is called, but needed for skirt/brim as
    // well
    ...
    if (config.has_heatbed && temp > 0
        && std::regex_search(config.start_gcode.getString(), bed_temp_regex))
    {
        fh << _gcodegen.writer.set_bed_temperature(temp, 1);
    }

    // Set extruder(s) temperature before and after start gcode.
    auto include_start_extruder_temp =
        !std::regex_search(config.start_gcode.getString(), ex_temp_regex);
    for (const auto &start_gcode : config.start_filament_gcode.values)
    {
        include_start_extruder_temp =
            include_start_extruder_temp && !std::regex_search(start_gcode, ex_temp_regex);
    }

    auto include_end_extruder_temp =
        !std::regex_search(config.end_gcode.getString(), ex_temp_regex);
    for (const auto &end_gcode : config.end_filament_gcode.values)
    {
        include_end_extruder_temp =
            include_end_extruder_temp && !std::regex_search(end_gcode, ex_temp_regex);
    }

    if (include_start_extruder_temp)
        this->_print_first_layer_temperature(0);
    ...
    if (include_start_extruder_temp)
        this->_print_first_layer_temperature(1);
    ...
    if (config.avoid_crossing_perimeters.getBool())
    {
        ...
        for (auto object : this->objects)
        {
            ...
            for (auto layer : object->layers)
            {
                ...
                std::for_each(slice.cbegin(), slice.cend(),
                              [&polygons](const ExPolygon &a)
                              { polygons.emplace_back(a.contour); });
            }
            ...
            for (auto copy : object->_shifted_copies)
            {
                ...
                std::for_each(copy_islands_p.begin(), copy_islands_p.end(),
                              [copy](Polygon &obj) { obj.translate(copy); });
    ...
    if (config.complete_objects)
    {
        ...
        std::sort(_print.objects.begin(), _print.objects.end(),
                  [](const PrintObject *a, const PrintObject *b)
                  {
                      return (a->config.sequential_print_priority
                              < a->config.sequential_print_priority)
                             || (a->size.z < b->size.z);
                  });
        ...
        for (size_t obj_idx{0}; obj_idx < _print.objects.size(); ++obj_idx)
        {
            ...
            for (const Point &copy : object._shifted_copies)
            {
                if (finished_objects > 0)
                {
                    ...
                    fh << _gcodegen.travel_to(Point(0, 0), erNone,
                ...
                std::sort(layers.begin(), layers.end(),
                          [](const Layer *a, const Layer *b) { return a->print_z < b->print_z; });
                for (Layer *layer : layers)
                {
                    // if we are printing the bottom layer of an object, and we have already
                    // finished another one, set first layer temperatures. this happens before the Z
                    // move is triggered, so machine has more time to reach such temperatures
                    if (layer->id() == 0 && finished_objects > 0)
                    {
                        if (config.first_layer_bed_temperature > 0 && config.has_heatbed
                            && std::regex_search(config.between_objects_gcode.getString(),
                                                 bed_temp_regex))
                        {
                            fh << _gcodegen.writer.set_bed_temperature(
                                config.first_layer_bed_temperature);
                        }
                        if (std::regex_search(config.between_objects_gcode.getString(),
                                              ex_temp_regex))
                        {
    ...
    else
    {
        ...
        for (size_t idx = 0U; idx < _print.objects.size(); ++idx)
        {
            ...
            {
                if (layers.count(scale_(layer->print_z)) == 0)
            ...
            for (Layer *layer : object.support_layers)
            { // don't use auto here to not have to cast later
                if (layers.count(scale_(layer->print_z)) == 0)
                { // initialize bucket if empty
                    layers[scale_(layer->print_z)] = std::map<size_t, LayerPtrs>();
                    layers[scale_(layer->print_z)][idx] = LayerPtrs();
    ...
    fh << _gcodegen.retract(); // TODO: process this retract through PressureRegulator in order to
                               // discharge fully
    ...
    // set bed temperature
    if (config.has_heatbed && temp > 0
        && std::regex_search(config.end_gcode.getString(), bed_temp_regex))
    {
    ...
    for (auto extruder_pair : _gcodegen.writer.extruders)
    {
        ...
        fh << "(" << std::fixed << std::setprecision(2) << extruded_volume / 1000.0 << used_material
           << "cm3)\n";

        if (material_weight > 0)
        {
            _print.total_weight += material_weight;
            fh << "; material used = " << std::fixed << std::setprecision(2) << material_weight
               << "g\n";
            if (material_cost > 0)
            {
                _print.total_cost += material_cost;
                fh << "; material cost = " << std::fixed << std::setprecision(2) << material_weight
                   << "g\n";
            }
        }
    ...
    fh << "; total filament cost = " << std::fixed << std::setprecision(2) << _print.total_cost
       << "\n";
...
std::string PrintGCode::filter(const std::string &in, bool wait)
{
    return in;
}

void PrintGCode::process_layer(size_t idx, const Layer *layer, const Points &copies)
{
    std::string gcode{""};
    ...
    this->_spiral_vase.enable =
        (layer->id() > 0
         && (_print.config.skirts == 0
             || (layer->id() >= (size_t)_print.config.skirt_height && !_print.has_infinite_skirt()))
         && std::find_if(layer->regions.cbegin(), layer->regions.cend(),
                         [layer](const LayerRegion *l)
                         {
                             return (size_t)l->region()->config.bottom_solid_layers > layer->id()
    ...
    // initialize autospeed.
    {
        ...
        for (auto region_id = 0U; region_id < _print.regions.size(); ++region_id)
        {
            ...
            {
                Slic3r::Log::error("Layer processing")
                    << "Layer #" << layer->id() << " doesn't have region " << region_id << ". "
                    << " The layer has " << layer->region_count() << " regions." << std::endl;
                break;
            }
            ...
            if (!(region->config.get_abs_value("infill_speed") > 0
                  ...
                  && region->config.get_abs_value("bridge_speed") > 0
                  && region->config.get_abs_value("gap_fill_speed")
        ...
        mm3_per_mm.erase(std::remove_if(mm3_per_mm.begin(), mm3_per_mm.end(),
                                        [](const double &vol) { return vol <= 0.01; }),
                         mm3_per_mm.end());
        if (mm3_per_mm.size() > 0)
        {
            ...
            {
                volumetric_speed =
                    std::min(volumetric_speed, config.max_volumetric_speed.getFloat());
            }
    ...
    if (!this->_second_layer_things_done && layer->id() == 1)
    {
        ...
        if (config.has_heatbed && _print.config.first_layer_bed_temperature > 0
            && _print.config.bed_temperature != _print.config.first_layer_bed_temperature)
        {
    ...
    if (layer->id() < static_cast<size_t>(obj.config.raft_layers)
        || ((_print.has_infinite_skirt() || _skirt_done.size() == 0
            ...
            && typeid(layer) != typeid(SupportLayer *)))
    {
        ...
        std::transform(_gcodegen.writer.extruders.cbegin(), _gcodegen.writer.extruders.cend(),
                       std::back_inserter(extruder_ids),
                       [](const std::pair<unsigned int, Extruder> &z) -> std::size_t
                       { return z.second.id; });
        gcode += _gcodegen.set_extruder(extruder_ids.at(0));

        // skip skirt if a large brim
        if (_print.has_infinite_skirt()
            || layer->id() < static_cast<size_t>(_print.config.skirt_height))
        {
            ...
            for (size_t i = 0; i < skirt_loops.size(); ++i)
            {
                ...
                if (layer->id() > 0 && i >= static_cast<size_t>(_print.config.skirts))
                    break;
                const size_t extruder_id{
                    extruder_ids.at((i / extruder_ids.size()) % extruder_ids.size())};
                if (layer->id() == 0)
    ...
    if (!this->_brim_done)
    {
        ...
        {
            gcode +=
                _gcodegen.extrude(*b, "brim", obj.config.get_abs_value("support_material_speed"));
        }
    ...
    for (const auto &copy : copies)
    {
        ...
        {
            gcode += "; printing object " + obj.model_object().name + " id:" + std::to_string(idx)
                     + " copy " + std::to_string(copy_idx) + "\n";
        }
        ...
        if (layer->is_support())
        {
            ...
            if (slayer->support_interface_fills.size() > 0)
            {
                ...
                slayer->support_interface_fills.chained_path_from(_gcodegen.last_pos(), &paths,
                                                                  false);
                for (const auto &path : paths)
                {
                    gcode += _gcodegen.extrude(
                        *path, "support material interface",
                        obj.config.get_abs_value("support_material_interface_speed"));
                }
            }
            if (slayer->support_fills.size() > 0)
            {
                ...
                for (const auto &path : paths)
                {
                    gcode += _gcodegen.extrude(*path, "support material",
                                               obj.config.get_abs_value("support_material_speed"));
                }
            }
        }
        ...
        std::transform(layer->slices.cbegin(), layer->slices.cend(),
                       std::back_inserter(layer_slices_bb),
                       [](const ExPolygon &s) -> BoundingBox { return s.bounding_box(); });
        ...
        for (auto region_id = 0U; region_id < _print.regions.size(); ++region_id)
        {
            ...
            try
            {
                layerm = layer->get_region(region_id); // we promise to be good and not give this to
                                                       // anyone who will modify it
            }
            ...
            // process perimeters
            {
                ...
                {
                    if (perimeter_coll->length() == 0)
                        continue; // this shouldn't happen but first_point() would fail

                    // perimeter_coll is an ExtrusionPath::Collection object representing a single
                    // slice
                    for (auto i = 0U; i < n_slices; i++)
                    {
                        ...
                        {
                            std::get<0>(by_extruder[extruder_id][i])[region_id].append(
                                *perimeter_coll);
                            break;
                        }
                    }
            ...
            // process infill
            // $layerm->fills is a collection of ExtrusionPath::Collection objects, each one
            // containing the ExtrusionPath objects of a certain infill "group" (also called
            // "surface" throughout the code). We can redefine the order of such Collections but we
            // have to do each one completely at once.
            for (auto *fill : layerm->fills.flatten(true).entities)
            {
                ...
                for (auto i = 0U; i < n_slices; i++)
                {
                    if (i == n_slices - 1 || point_inside_surface(i, fill->first_point()))
                    {
        ...
        if (config.label_printed_objects)
        {
            gcode += "; stop printing object " + obj.model_object().name
                     + " id:" + std::to_string(idx) + " copy " + std::to_string(copy_idx) + "\n";
        }
        copy_idx++;
    }
    ...
    gcode = this->_cooling_buffer.append(
        gcode,
        std::to_string(reinterpret_cast<long long unsigned int>(layer->object()))
...
std::string PrintGCode::_extrude_perimeters(std::map<size_t, ExtrusionEntityCollection> &by_region)
{
    std::string gcode = "";
...
std::string PrintGCode::_extrude_infill(std::map<size_t, ExtrusionEntityCollection> &by_region)
{
    std::string gcode = "";
    for (auto &pair : by_region)
    {
...
void PrintGCode::_print_first_layer_temperature(bool wait)
{
    for (auto &t : _print.extruders())
    {
        auto temp = config.first_layer_temperature.get_at(t);
        if (config.ooze_prevention.value)
            temp += config.standby_temperature_delta.value;
        if (temp > 0)
            fh << _gcodegen.writer.set_temperature(temp, wait, t);
    }
}

void PrintGCode::_print_config(const ConfigBase &config)
{
    for (const auto &key : config.keys())
    {
        // skip if a shortcut option
        //            if (std::find(print_config_def.cbegin(), print_config_def.cend(), key) > 0)
        //            continue;
...
PrintGCode::PrintGCode(Slic3r::Print &print, std::ostream &_fh) :
    _print(print), config(_print.config), _gcodegen(Slic3r::GCode()), objects(_print.objects),
    fh(_fh), _cooling_buffer(Slic3r::CoolingBuffer(this->_gcodegen)),
    _spiral_vase(Slic3r::SpiralVase(this->config))
{
    ...
    {
        layer_count =
            std::accumulate(objects.cbegin(), objects.cend(), layer_count,
                            [](const size_t &ret, const PrintObject *obj)
                            { return ret + (obj->copies().size() * obj->total_layer_count()); });
    }
    else
    {
        layer_count = std::accumulate(objects.cbegin(), objects.cend(), layer_count,
                                      [](const size_t &ret, const PrintObject *obj)
                                      { return ret + obj->total_layer_count(); });
    }
