...
bool TMFEditor::write_relationships()
{
...
bool TMFEditor::write_object(boost::nowide::ofstream &fout, const ModelObject *object, int index)
{
    ...
    {
        fout << "        <slic3r:object type=\"" << key << "\" config=\""
             << object->config.serialize(key) << "\""
             << "/>\n";
    }
    ...
    // Save the start offset (triangle offset) of each volume (To be saved for writing Slic3r custom
    // configs).
    ...
    for (const auto volume : object->volumes)
    {
        ...
        {
            fout << "                        <slic3r:metadata type=\"" << key << "\" config=\""
...
bool TMFEditor::write_build(boost::nowide::ofstream &fout)
{
    ...
    for (const auto object : model->objects)
    {
        for (const auto instance : object->instances)
        {
            ...
            // Add the transform
            fout << " transform=\"" << trafo.m00 << " " << trafo.m10 << " " << trafo.m20 << " "
                 << trafo.m01 << " " << trafo.m11 << " " << trafo.m21 << " " << trafo.m02 << " "
                 << trafo.m12 << " " << trafo.m22 << " " << trafo.m03 << " " << trafo.m13 << " "
                 << trafo.m23 << "\"/>\n";
        }
        object_id++;
    }
...
bool TMFEditor::read_model()
{
    ...
    if (!fin.is_open())
    {
        boost::nowide::cerr << "Cannot open file: "
                            << "3dmodel.model" << std::endl;
        return false;
    }
...
bool TMF::write(const Model &model, std::string output_file)
{
    Model m2{model};
...
bool TMF::read(std::string input_file, Model *model)
{
    if (!model)
        return false;
...
TMFParserContext::TMFParserContext(XML_Parser parser, Model *model) :
    m_parser(parser), m_path(std::vector<TMFNodeType>()), m_model(*model), m_object(nullptr),
    m_objects_indices(std::map<std::string, int>()), m_output_objects(std::vector<bool>()),
    m_object_vertices(std::vector<float>()), m_volume(nullptr), m_volume_facets(std::vector<int>())
{
    m_path.reserve(9);
    m_value[0] = m_value[1] = m_value[2] = "";
}

void XMLCALL TMFParserContext::startElement(void *userData, const char *name, const char **atts)
{
    TMFParserContext *ctx = (TMFParserContext *)userData;
    ctx->startElement(name, atts);
}

void XMLCALL TMFParserContext::endElement(void *userData, const char *name)
{
    TMFParserContext *ctx = (TMFParserContext *)userData;
    ctx->endElement();
}

void XMLCALL TMFParserContext::characters(void *userData, const XML_Char *s, int len)
{
    TMFParserContext *ctx = (TMFParserContext *)userData;
    ctx->characters(s, len);
}

const char *TMFParserContext::get_attribute(const char **atts, const char *id)
{
    if (atts == NULL)
        return NULL;
    while (*atts != NULL)
...
void TMFParserContext::startElement(const char *name, const char **atts)
{
    ...
    case 5:
        ...
        else if (strcmp(name, "slic3r:volume") == 0)
        {
            ...
            if (!m_object)
                this->stop();
            m_volume = add_volume(stoi(m_value[0]) * 3, stoi(m_value[1]) * 3 + 2,
                                  static_cast<bool>(stoi(m_value[2])));
            if (!m_volume)
                this->stop();
...
void TMFParserContext::endElement()
{
    switch (m_path.back())
    {
...
void TMFParserContext::characters(const XML_Char *s, int len)
{
    switch (m_path.back())
    {
    case NODE_TYPE_METADATA:
...
void TMFParserContext::endDocument()
{
}

void TMFParserContext::stop()
{
    XML_StopParser(m_parser, 0);
}

bool TMFParserContext::extract_trafo(std::string matrix, TransformationMatrix &trafo)
{
    // Get the values.
    double m[12];
    int k = 0;
...
ModelVolume *TMFParserContext::add_volume(int start_offset, int end_offset, bool modifier)
{
    ModelVolume *m_volume = nullptr;

    // Add a new volume.
    m_volume = m_object->add_volume(TriangleMesh());
    if (!m_volume || (end_offset < start_offset))
        return nullptr;
    ...
    for (int i = start_offset; i <= end_offset;)
    {
        ...
        {
            memcpy(&facet.vertex[v].x, &m_object_vertices[m_volume_facets[i++] * 3],
                   3 * sizeof(float));
            i_facet++;
        }
    }
