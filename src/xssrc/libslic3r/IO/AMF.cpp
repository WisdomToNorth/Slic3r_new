...
bool extract_model_from_archive(mz_zip_archive &archive, const mz_zip_archive_file_stat &stat,
                                Model *model, bool check_version);

struct AMFParserContext
{
    AMFParserContext(XML_Parser parser, Model *model) :
        m_parser(parser), m_model(*model), m_object(NULL), m_volume(NULL), m_material(NULL),
        m_instance(NULL)
    {
        m_path.reserve(12);
    }
    ...
    case 2:
        ...
        else if (strcmp(name, "instance") == 0)
        {
            if (m_path[1] == NODE_TYPE_CONSTELLATION)
            {
                ...
                {
                    m_object_instances_map[object_id].instances.push_back(
                        AMFParserContext::Instance());
...
void AMFParserContext::characters(const XML_Char *s, int len)
{
    ...
    else
    {
        ...
        {
        case 4:
            if (m_path.back() == NODE_TYPE_DELTAX || m_path.back() == NODE_TYPE_DELTAY
                || m_path.back() == NODE_TYPE_RZ || m_path.back() == NODE_TYPE_SCALE)
...
bool AMF::read(std::string input_file, Model *model)
{
...
bool AMF::write(const Model &model, std::string output_file)
{
    using namespace std;
    ...
    for (const auto &material : model.materials)
    {
        ...
        for (const auto &attr : material.second->attributes)
            file << "    <metadata type=\"" << attr.first << "\">" << attr.second << "</metadata>"
                 << endl;
    ...
    for (size_t object_id = 0; object_id < model.objects.size(); ++object_id)
    {
        ...
        for (const std::string &key : object->config.keys())
            file << "    <metadata type=\"slic3r." << key << "\">" << object->config.serialize(key)
                 << "</metadata>" << endl;
        ...
        for (ModelVolume *volume : object->volumes)
        {
            ...
            for (size_t i = 0; i < static_cast<size_t>(stl.stats.shared_vertices); ++i)
                ...
                file << "         <vertex>" << endl
                     << "           <coordinates>" << endl
                     << "             <x>" << (stl.v_shared[i].x - origin_translation.x) << "</x>"
                     << endl
                     << "             <y>" << (stl.v_shared[i].y - origin_translation.y) << "</y>"
                     << endl
                     << "             <z>" << (stl.v_shared[i].z - origin_translation.z) << "</z>"
                     << endl
                     << "           </coordinates>" << endl
                     << "         </vertex>" << endl;
        ...
        for (size_t i_volume = 0; i_volume < object->volumes.size(); ++i_volume)
        {
            ...
            for (int i = 0; i < volume->mesh.stl.stats.number_of_facets; ++i)
            {
                ...
                for (int j = 0; j < 3; ++j)
                    file << "          <v" << (j + 1) << ">"
                         << (volume->mesh.stl.v_indices[i].vertex[j] + vertices_offset) << "</v"
                         << (j + 1) << ">" << endl;
                file << "        </triangle>" << endl;
            }
            file << "      </volume>" << endl;
        ...
        for (const ModelInstance *instance : object->instances)
            instances << "    <instance objectid=\"" << object_id << "\">" << endl
                      << "      <deltax>" << instance->offset.x + origin_translation.x
                      << "</deltax>" << endl
                      << "      <deltay>" << instance->offset.y + origin_translation.y
                      << "</deltay>" << endl
    ...
    if (!instances_str.empty())
        file << "  <constellation id=\"1\">" << endl
             << instances_str << "  </constellation>" << endl;
                                ...
                                Model *model, bool check_version)
{
    ...
    struct CallbackData
    {
        ...
        CallbackData(XML_Parser &parser, const mz_zip_archive_file_stat &stat) :
            parser(parser), stat(stat)
        {
        }
    };
...
bool load_amf_archive(const char *path, Model *model, bool check_version)
{
    ...
    // we first loop the entries to read from the archive the .amf file only, in order to extract
    // the version from it
