...
GCodeSender::GCodeSender() :
    io(), serial(io), open(false), connected(false), error(false), can_send(false),
    queue_paused(false), sent(0)
{
}
...
bool GCodeSender::connect(std::string devname, unsigned int baud_rate)
{
    ...
    try
    {
        ...
        this->serial.set_option(
            asio::serial_port_base::character_size(asio::serial_port_base::character_size(8)));
        this->serial.set_option(
    ...
    std::thread t(boost::bind(&asio::io_service::run, &this->io));
...
void GCodeSender::set_baud_rate(unsigned int baud_rate)
{
    ...
    catch (boost::system::system_error &e)
    {
        ...
        ios.c_cc[VMIN] =
            1; // Minimum of characters to read, prevents eof errors when 0 bytes are read
        ios.c_cc[VTIME] = 1;
...
bool GCodeSender::is_connected() const
{
...
{
    std::lock_guard<std::mutex> l(this->log_mutex);
...
{
    std::lock_guard<std::mutex> l(this->error_mutex);
    this->error = e;
}
...
{
    // read one line
    asio::async_read_until(this->serial, this->read_buffer, '\n',
                           boost::bind(&GCodeSender::on_read, this, asio::placeholders::error,
...
void GCodeSender::on_read(const boost::system::error_code &error, size_t bytes_transferred)
{
    ...
    if (!line.empty())
    {
        ...
        // parse incoming line
        if (!this->connected
            && (boost::starts_with(line, "start") || boost::starts_with(line, "Grbl ")
                || boost::starts_with(line, "ok") || boost::contains(line, "T:")))
        {
            this->connected = true;
            {
                std::lock_guard<std::mutex> l(this->queue_mutex);
                 ...
                 || boost::istarts_with(line, "rs"))
        {
            ...
            size_t toresend =
                boost::lexical_cast<size_t>(line.substr(0, line.find_first_not_of("0123456789")));
            toresend++; // N is 0-based
            if (toresend >= this->sent - this->last_sent.size()
                && toresend < this->last_sent.size())
            {
                {
                    std::lock_guard<std::mutex> l(this->queue_mutex);
                    ...
                    this->priqueue.insert(this->priqueue.begin(), // insert at the beginning
                                          this->last_sent.begin() + toresend
                                              - (this->sent - this->last_sent.size()) - 1,
                                          this->last_sent.end());
        ...
        // parse temperature info
        {
            ...
            if (pos != std::string::npos && line.size() > pos + 2)
            {
                ...
                std::lock_guard<std::mutex> l(this->log_mutex);
                this->T = line.substr(pos + 2,
                                      line.find_first_not_of("0123456789.", pos + 2) - (pos + 2));

                pos = line.find("B:");
                if (pos != std::string::npos && line.size() > pos + 2)
                {
                    // we got bed temperature info
                    this->B = line.substr(pos + 2, line.find_first_not_of("0123456789.", pos + 2)
                                                       - (pos + 2));
                }
            }
        }
...
void GCodeSender::send(const std::vector<std::string> &lines, bool priority)
{
    ...
    {
        std::lock_guard<std::mutex> l(this->queue_mutex);
        for (std::vector<std::string>::const_iterator line = lines.begin(); line != lines.end();
             ++line)
        {
            if (priority)
            {
...
void GCodeSender::do_send()
{
    ...
    os << full_line;
    asio::async_write(this->serial, this->write_buffer,
