...
const std::map<ExportFormat, std::string> extensions{
    {STL, "stl"}, {OBJ, "obj"}, {POV, "pov"},     {AMF, "amf"},
    {TMF, "3mf"}, {SVG, "svg"}, {Gcode, "gcode"},
};

const std::map<ExportFormat, bool (*)(const Model &, std::string)> write_model{
    {STL, &STL::write}, {OBJ, &OBJ::write}, {POV, &POV::write},
    {AMF, &AMF::write}, {TMF, &TMF::write},
};

bool STL::read(std::string input_file, TriangleMesh *mesh)
{
...
bool STL::read(std::string input_file, Model *model)
{
    TriangleMesh mesh;
    if (!STL::read(input_file, &mesh))
        return false;
...
bool STL::write(const Model &model, std::string output_file, bool binary)
{
    TriangleMesh mesh = model.mesh();
    return STL::write(mesh, output_file, binary);
}

bool STL::write(const TriangleMesh &mesh, std::string output_file, bool binary)
{
    if (binary)
    {
...
bool OBJ::read(std::string input_file, TriangleMesh *mesh)
{
    Model model;
...
bool OBJ::read(std::string input_file, Model *model)
{
         ...
         shape != shapes.end(); ++shape)
    {
        ...
        {
            points.push_back(
                Pointf3(attrib.vertices[v], attrib.vertices[v + 1], attrib.vertices[v + 2]));
        }
        ...
        for (size_t f = 0; f < shape->mesh.num_face_vertices.size(); ++f)
        {
            ...
            facets.push_back(Point3(shape->mesh.indices[f * 3 + 0].vertex_index,
...
bool OBJ::write(const Model &model, std::string output_file)
{
    TriangleMesh mesh = model.mesh();
...
bool OBJ::write(const TriangleMesh &mesh, std::string output_file)
{
    mesh.WriteOBJFile(output_file);
    return true;
}

bool POV::write(const Model &model, std::string output_file)
{
    TriangleMesh mesh{model.mesh()};
    return POV::write(mesh, output_file);
}

bool POV::write(const TriangleMesh &mesh, std::string output_file)
{
    using namespace std;
