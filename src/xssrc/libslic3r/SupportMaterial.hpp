...
struct toolpaths_params
{
    ...
    toolpaths_params(int contact_loops = 0, coordf_t circle_radius = 0,
                     coordf_t circle_distance = 0, const Polygon &circle = Polygon(),
                     const SupportMaterialPattern &pattern = SupportMaterialPattern(),
                     const vector<int> &angles = vector<int>()) :
        contact_loops(contact_loops),
        circle_radius(circle_radius), circle_distance(circle_distance), circle(circle),
        pattern(pattern), angles(angles)
    {
    }
};
...
public:
    ...
    /// Generate the extrusions paths for the support matterial generated for the given print
    /// object.
    void generate_toolpaths(PrintObject *object, map<coordf_t, Polygons> overhang,
                            map<coordf_t, Polygons> contact, map<int, Polygons> _interface,
    ...
    vector<coordf_t> support_layers_z(vector<coordf_t> contact_z, vector<coordf_t> top_z,
    ...
    void generate_pillars_shape(const map<coordf_t, Polygons> &contact,
                                const vector<coordf_t> &support_z, map<int, Polygons> &shape);
    ...
    void generate_bottom_interface_layers(const vector<coordf_t> &support_z,
                                          map<int, Polygons> &base, map<coordf_t, Polygons> &top,
                                          map<int, Polygons> &_interface);
    ...
    void clip_with_object(map<int, Polygons> &support, vector<coordf_t> support_z,
                          PrintObject &object);
...
private:
    ...
    SupportMaterial(PrintConfig *print_config, PrintObjectConfig *print_object_config, Flow flow,
                    Flow first_layer_flow, Flow interface_flow) :
        config(print_config),
        object_config(print_object_config), flow(Flow(0, 0, 0)), first_layer_flow(Flow(0, 0, 0)),
        interface_flow(Flow(0, 0, 0)), object(nullptr)
    {
    }
