...
PrintConfigDef::PrintConfigDef()
{
    ...
    def->tooltip =
        __TRANS("Unselecting this will suppress automatic generation of bed heating gcode.");
    def->cli = "has_heatbed!";
    ...
    def->tooltip = __TRANS(
        "This disables the fan completely for the first N layers to aid in the adhesion of media to the bed.");
    ...
    def->category = __TRANS("Infill");
    def->tooltip = __TRANS(
        "Fill pattern for top/bottom infill. This only affects the external visible layer, and not its adjacent solid shells.");
    ...
    def->tooltip =
        __TRANS("The extruder to use (unless more specific extruder settings are specified).");
    def->cli = "extruder=i";
    ...
    def->tooltip =
        __TRANS("Set this if your printer uses control values from 0-100 instead of 0-255.");
    ...
    def->tooltip =
        __TRANS("Enter your filament cost per kg here. This is only for statistical information.");
    ...
    def->tooltip =
        "Enable this to add comments in the G-Code that label print moves with what object they belong. Can be used with Octoprint CancelObject plugin.";
    ...
    def->tooltip = __TRANS(
        "This custom code is inserted at every layer change, right after the Z move and before the extruder moves to the first layer point. Note that you can use placeholder variables for all Slic3r settings as well as [layer_num], [layer_z] and [current_retraction].");
    ...
    def->sidetext = "mm";
    def->tooltip =
        "Alternative method of configuring perimeters and top/bottom layers. If this is above 0 extra perimeters and solid layers will be generated when necessary";
    ...
    def->sidetext = "mm";
    def->tooltip =
        "Alternative method of configuring top/bottom layers. If this is above 0 extra solid layers will be generated when necessary";
    ...
    def->tooltip =
        __TRANS("This is the diameter of your extruder nozzle (for example: 0.5, 0.35 etc.)");
    def->sidetext = "mm";
    ...
    def->tooltip = __TRANS(
        "Slic3r can upload G-code files to OctoPrint. This field should contain the API Key required for authentication.");
    ...
    def->tooltip = __TRANS(
        "Slic3r can upload G-code files to an Octoprint/Duet server. This field should contain the hostname or IP address of the server instance.");
    ...
    def->category = __TRANS("Extruders");
    def->tooltip =
        __TRANS("The extruder to use when printing perimeters and brim. First extruder is 1.");
    ...
    def->tooltip =
        __TRANS("Retraction is not triggered when travel moves are shorter than this length.");
    def->sidetext = "mm";
    ...
    def->category = __TRANS("Support material");
    def->tooltip =
        __TRANS("Use this setting to rotate the support material pattern on the horizontal plane.");
    def->sidetext = "°";
    ...
    def->category = __TRANS("Support material");
    def->tooltip =
        __TRANS("Number of interface layers to insert between the object(s) and support material.");
    def->sidetext = __TRANS("layers");
...
void DynamicPrintConfig::normalize()
{
    ...
    */
    ...
    if (!this->has("solid_infill_extruder") && this->has("infill_extruder"))
        this->option("solid_infill_extruder", true)
            ->setInt(this->option("infill_extruder")->getInt());
...
double PrintConfigBase::min_object_distance() const
{
    ...
    return (this->option("complete_objects")->getBool()
