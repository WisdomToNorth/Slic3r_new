...
inline Points &operator+=(Points &dst, const Points &src)
{
    append_to(dst, src);
    return dst;
};

inline Points &operator+=(Points &dst, const Point &p)
{
    dst.push_back(p);
    return dst;
};
...
public:
    coord_t z;
    explicit constexpr Point3(coord_t _x = 0, coord_t _y = 0, coord_t _z = 0) :
        Point(_x, _y), z(_z){};
...
public:
    coordf_t z;
    explicit constexpr Pointf3(coordf_t _x = 0, coordf_t _y = 0, coordf_t _z = 0) :
        Pointf(_x, _y), z(_z){};
...
template <class T>
inline Points to_points(const std::vector<T> &items)
{
    Points pp;
...
inline Points scale(const std::vector<Pointf> &in)
{
    Points out;
    for (const auto &p : in)
    {
        out.push_back(Point(scale_(p.x), scale_(p.y)));
    }
    return out;
}
...
#ifndef _WIN32
template <>
struct geometry_concept<coord_t>
{
