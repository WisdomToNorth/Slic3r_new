...
TriangleMesh::TriangleMesh(const TriangleMesh &other) : stl(other.stl), repaired(other.repaired)
{
    this->clone(other);
}
...
void TriangleMesh::clone(const TriangleMesh &other)
{
    ...
    if (other.stl.facet_start != NULL)
    {
        ...
        std::copy(other.stl.facet_start, other.stl.facet_start + other.stl.stats.number_of_facets,
                  this->stl.facet_start);
    }
    if (other.stl.neighbors_start != NULL)
    {
        this->stl.neighbors_start =
            (stl_neighbors *)calloc(other.stl.stats.number_of_facets, sizeof(stl_neighbors));
        std::copy(other.stl.neighbors_start,
                  other.stl.neighbors_start + other.stl.stats.number_of_facets,
                  this->stl.neighbors_start);
    }
    if (other.stl.v_indices != NULL)
    {
        this->stl.v_indices =
            (v_indices_struct *)calloc(other.stl.stats.number_of_facets, sizeof(v_indices_struct));
        std::copy(other.stl.v_indices, other.stl.v_indices + other.stl.stats.number_of_facets,
                  this->stl.v_indices);
    }
    if (other.stl.v_shared != NULL)
    {
        this->stl.v_shared =
            (stl_vertex *)calloc(other.stl.stats.shared_vertices, sizeof(stl_vertex));
        std::copy(other.stl.v_shared, other.stl.v_shared + other.stl.stats.shared_vertices,
                  this->stl.v_shared);
    }
}
...
void TriangleMesh::ReadSTLFile(const std::string &input_file)
{
#ifdef BOOST_WINDOWS
...
void TriangleMesh::write_ascii(const std::string &output_file) const
{
#ifdef BOOST_WINDOWS
...
void TriangleMesh::write_binary(const std::string &output_file) const
{
#ifdef BOOST_WINDOWS
...
void TriangleMesh::check_topology()
{
    ...
    if (stl.stats.connected_facets_3_edge < stl.stats.number_of_facets)
    {
        for (int i = 0; i < iterations; i++)
        {
            if (stl.stats.connected_facets_3_edge < stl.stats.number_of_facets)
            {
                // printf("Checking nearby. Tolerance= %f Iteration=%d of %d...", tolerance, i + 1,
                // iterations);
...
{
    return this->stl.stats.degenerate_facets > 0 || this->stl.stats.edges_fixed > 0
           || this->stl.stats.facets_removed > 0 || this->stl.stats.facets_added > 0
           || this->stl.stats.facets_reversed > 0 || this->stl.stats.backwards_edges > 0;
}

size_t TriangleMesh::facets_count() const
{
    return this->stl.stats.number_of_facets;
}

void TriangleMesh::WriteOBJFile(const std::string &output_file) const
{
...
{
    this->translate(static_cast<float>(vec.x), static_cast<float>(vec.y),
...
{
    this->translate(-(this->stl.stats.min.x), -(this->stl.stats.min.y), -(this->stl.stats.min.z));
}

void TriangleMesh::center_around_origin()
{
    this->align_to_origin();
    this->translate(-(this->stl.stats.size.x / 2), -(this->stl.stats.size.y / 2),
...
Pointf3 TriangleMesh::center() const
{
...
mesh_stats TriangleMesh::stats() const
{
...
void TriangleMesh::cut(Axis axis, double z, TriangleMesh *upper, TriangleMesh *lower)
{
    switch (axis)
    {
    case X: TriangleMeshSlicer<X>(this).cut(z, upper, lower); break;
    case Y: TriangleMeshSlicer<Y>(this).cut(z, upper, lower); break;
    case Z: TriangleMeshSlicer<Z>(this).cut(z, upper, lower); break;
    default: Slic3r::Log::error("TriangleMesh", "Invalid Axis supplied to cut()");
    }
}
...
TriangleMeshPtrs TriangleMesh::cut_by_grid(const Pointf &grid) const
{
...
void TriangleMesh::merge(const TriangleMesh &mesh)
{
    ...
    // copy facets
    std::copy(mesh.stl.facet_start, mesh.stl.facet_start + mesh.stl.stats.number_of_facets,
              this->stl.facet_start + number_of_facets);
    std::copy(mesh.stl.neighbors_start, mesh.stl.neighbors_start + mesh.stl.stats.number_of_facets,
              this->stl.neighbors_start + number_of_facets);
...
ExPolygons TriangleMesh::horizontal_projection() const
{
    ...
    for (int i = 0; i < this->stl.stats.number_of_facets; i++)
    {
        ...
        p.points.resize(3);
        p.points[0] =
            Point(facet->vertex[0].x / SCALING_FACTOR, facet->vertex[0].y / SCALING_FACTOR);
        p.points[1] =
            Point(facet->vertex[1].x / SCALING_FACTOR, facet->vertex[1].y / SCALING_FACTOR);
        p.points[2] =
            Point(facet->vertex[2].x / SCALING_FACTOR, facet->vertex[2].y / SCALING_FACTOR);
        p.make_counter_clockwise(); // do this after scaling, as winding order might change while
                                    // doing that
        pp.push_back(p);
    }
...
BoundingBoxf3 TriangleMesh::get_transformed_bounding_box(TransformationMatrix const &trafo) const
{
    BoundingBoxf3 bbox;
    for (int i = 0; i < this->stl.stats.number_of_facets; ++i)
    {
...
void TriangleMesh::reverse_normals()
{
    stl_reverse_all_facets(&this->stl);
    if (this->stl.stats.volume != -1)
        this->stl.stats.volume *= -1.0;
}

void TriangleMesh::extrude_tin(float offset)
{
...
TriangleMesh TriangleMesh::make_cube(double x, double y, double z)
{
    Pointf3 pv[8] = {Pointf3(x, y, 0), Pointf3(x, 0, 0), Pointf3(0, 0, 0), Pointf3(0, y, 0),
                     Pointf3(x, y, z), Pointf3(0, y, z), Pointf3(0, 0, z), Pointf3(x, 0, z)};
    Point3 fv[12] = {Point3(0, 1, 2), Point3(0, 2, 3), Point3(4, 5, 6), Point3(4, 6, 7),
                     Point3(0, 4, 7), Point3(0, 7, 1), Point3(1, 7, 6), Point3(1, 6, 2),
                     Point3(2, 6, 5), Point3(2, 5, 3), Point3(4, 0, 3), Point3(4, 3, 5)};
...
TriangleMesh TriangleMesh::make_cylinder(double r, double h, double fa)
{
    Pointf3s vertices;
...
TriangleMesh TriangleMesh::make_sphere(double rho, double fa)
{
    Pointf3s vertices;
    std::vector<Point3> facets;
...
template <Axis A>
void TriangleMeshSlicer<A>::slice(const std::vector<float> &z, std::vector<Polygons> *layers) const
{
    /**
    ...
    */
    ...
    std::vector<IntersectionLines> lines(z.size());
    {
        ...
        parallelize<int>(
            0, this->mesh->stl.stats.number_of_facets - 1,
...
template <Axis A>
void TriangleMeshSlicer<A>::_slice_do(size_t facet_idx, std::vector<IntersectionLines> *lines,
                                      std::mutex *lines_mutex, const std::vector<float> &z) const
{
...
#ifdef SLIC3R_DEBUG
    printf("\n==> FACET %zu (%f,%f,%f - %f,%f,%f - %f,%f,%f):\n", facet_idx, _x(facet.vertex[0]),
           _y(facet.vertex[0]), _z(facet.vertex[0]), _x(facet.vertex[1]), _y(facet.vertex[1]),
           _z(facet.vertex[1]), _x(facet.vertex[2]), _y(facet.vertex[2]), _z(facet.vertex[2]));
    ...
    min_layer =
        std::lower_bound(z.begin(), z.end(), min_z); // first layer whose slice_z is >= min_z
    max_layer = std::upper_bound(z.begin() + (min_layer - z.begin()), z.end(), max_z)
                - 1; // last layer whose slice_z is <= max_z
#ifdef SLIC3R_DEBUG
    printf("layers: min = %d, max = %d\n", (int)(min_layer - z.begin()),
           (int)(max_layer - z.begin()));
#endif

    for (std::vector<float>::const_iterator it = min_layer; it != max_layer + 1; ++it)
    {
        std::vector<float>::size_type layer_idx = it - z.begin();
        this->slice_facet(*it / SCALING_FACTOR, facet, facet_idx, min_z, max_z,
                          &(*lines)[layer_idx], lines_mutex);
    }
}

template <Axis A>
void TriangleMeshSlicer<A>::slice(const std::vector<float> &z,
                                  std::vector<ExPolygons> *layers) const
{
    std::vector<Polygons> layers_p;
    this->slice(z, &layers_p);

    layers->resize(z.size());
    for (std::vector<Polygons>::const_iterator loops = layers_p.begin(); loops != layers_p.end();
         ++loops)
    {
#ifdef SLIC3R_DEBUG
...
template <Axis A>
void TriangleMeshSlicer<A>::slice(float z, ExPolygons *slices) const
{
    std::vector<float> zz;
    zz.push_back(z);
    std::vector<ExPolygons> layers;
...
template <Axis A>
void TriangleMeshSlicer<A>::slice_facet(float slice_z, const stl_facet &facet, const int &facet_idx,
                                        ...
                                        std::mutex *lines_mutex) const
{
    ...
    { // loop through facet edges
        ...
        if (_z(*a) == _z(*b) && _z(*a) == slice_z)
        {
            ...
            if (line.edge_type != feHorizontal)
                return;
        }
...
template <Axis A>
void TriangleMeshSlicer<A>::_make_loops_do(size_t i, std::vector<IntersectionLines> *lines,
...
template <Axis A>
void TriangleMeshSlicer<A>::make_loops(std::vector<IntersectionLine> &lines, Polygons *loops) const
{
    /*
    SVG svg("lines.svg");
    svg.draw(lines);
...
CYCLE:
    while (1)
    {
        ...
        /*
        printf("first_line edge_a_id = %d, edge_b_id = %d, a_id = %d, b_id = %d, a = %d,%d, b =
        ...
        */
        ...
        while (1)
        {
            ...
            if (loop.back()->edge_b_id != -1)
            {
                ...
                for (IntersectionLinePtrs::iterator lineptr = candidates.begin();
                     lineptr != candidates.end(); ++lineptr)
                {
                    if ((*lineptr)->skip)
                        continue;
                    next_line = *lineptr;
            ...
            if (next_line == NULL)
            {
                ...
                if ((loop.front()->edge_a_id != -1
                     && loop.front()->edge_a_id == loop.back()->edge_b_id)
                    || (loop.front()->a_id != -1 && loop.front()->a_id == loop.back()->b_id))
                {
                    // loop is complete
                    Polygon p;
                    p.points.reserve(loop.size());
                    for (IntersectionLinePtrs::const_iterator lineptr = loop.begin();
                         lineptr != loop.end(); ++lineptr)
                    {
            ...
            /*
            printf("next_line edge_a_id = %d, edge_b_id = %d, a_id = %d, b_id = %d, a = %d,%d, b =
...
template <Axis A>
void TriangleMeshSlicer<A>::make_expolygons_simple(std::vector<IntersectionLine> &lines,
                                                   ExPolygons *slices) const
{
    Polygons loops;
...
template <Axis A>
void TriangleMeshSlicer<A>::make_expolygons(const Polygons &loops, ExPolygons *slices) const
{
    /**
    ...
    */
    ...
    Polygons p_slices;
    for (std::vector<size_t>::const_iterator loop_idx = sorted_area.begin();
         loop_idx != sorted_area.end(); ++loop_idx)
    {
...
template <Axis A>
void TriangleMeshSlicer<A>::make_expolygons(std::vector<IntersectionLine> &lines,
                                            ExPolygons *slices) const
{
    Polygons pp;
...
template <Axis A>
void TriangleMeshSlicer<A>::cut(float z, TriangleMesh *upper, TriangleMesh *lower) const
{
    ...
    for (int facet_idx = 0; facet_idx < this->mesh->stl.stats.number_of_facets; facet_idx++)
    {
        ...
        // find facet extents
        float min_z =
            fminf(_z(facet->vertex[0]), fminf(_z(facet->vertex[1]), _z(facet->vertex[2])));
        float max_z =
            fmaxf(_z(facet->vertex[0]), fmaxf(_z(facet->vertex[1]), _z(facet->vertex[2])));
    ...
    if (upper != NULL)
    {
        ...
        // triangulate section
        Polygons triangles;
        for (ExPolygons::const_iterator expolygon = section.begin(); expolygon != section.end();
             ++expolygon)
            expolygon->triangulate_p2t(&triangles);

        // convert triangles to facets and append them to mesh
        for (Polygons::const_iterator polygon = triangles.begin(); polygon != triangles.end();
             ++polygon)
        {
            Polygon p = *polygon;
            p.reverse();
...
TriangleMeshSlicer<A>::TriangleMeshSlicer(TriangleMesh *_mesh) : mesh(_mesh), v_scaled_shared(NULL)
{
    ...
    {
        t_edges edges;
        // reserve() instead of resize() because otherwise we couldn't read .size() below to assign
        // edge_idx
        edges.reserve(this->mesh->stl.stats.number_of_facets
                      * 3); // number of edges = number of facets * 3
    ...
    this->v_scaled_shared =
        (stl_vertex *)calloc(this->mesh->stl.stats.shared_vertices, sizeof(stl_vertex));
    std::copy(this->mesh->stl.v_shared,
              this->mesh->stl.v_shared + this->mesh->stl.stats.shared_vertices,
              this->v_scaled_shared);
