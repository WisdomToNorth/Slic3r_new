...
std::string escape_strings_cstyle(const std::vector<std::string> &strs)
{
    ...
    for (size_t j = 0; j < strs.size(); ++j)
    {
        ...
        // Is the string simple or complex? Complex string contains spaces, tabs, new lines and
        // other escapable characters. Empty string shall be quoted as well, if it is the only
        // string in strs.
...
ConfigOptionDef::ConfigOptionDef(const ConfigOptionDef &other) :
    type(other.type), default_value(nullptr), gui_type(other.gui_type), gui_flags(other.gui_flags),
    label(other.label), full_label(other.full_label), category(other.category),
    tooltip(other.tooltip), sidetext(other.sidetext), cli(other.cli), ratio_over(other.ratio_over),
...
ConfigOptionDef *ConfigDef::add(const t_config_option_key &opt_key, const ConfigOptionDef &def)
{
    this->options.insert(std::make_pair(opt_key, def));
    return &this->options[opt_key];
}

bool ConfigDef::has(const t_config_option_key &opt_key) const
{
...
void ConfigDef::merge(const ConfigDef &other)
{
...
bool ConfigBase::has(const t_config_option_key &opt_key) const
{
...
void ConfigBase::apply_only(const ConfigBase &other, const t_config_option_keys &opt_keys,
...
void ConfigBase::apply_only(const ConfigBase &other, const t_config_option_keys &opt_keys,
                            bool ignore_nonexistent, bool default_nonexistent)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys)
    {
        ...
        catch (UnknownOptionException &e)
        {
            if (!ignore_nonexistent)
                Slic3r::Log::warn("Config")
                    << "Option " << opt_key << ": " << e.what() << std::endl;
        }
        if (opt_key.size() == 0)
            continue;
        if (my_opt == NULL)
        ...
        if (default_nonexistent && !other.has(opt_key))
        {
            ...
            {
                std::string error =
                    "Unexpected failure when deserializing serialized value for " + opt_key;
                CONFESS(error.c_str());
            }
            continue;
        }
...
void ConfigBase::set_defaults(const t_config_option_keys &opt_keys)
{
...
bool ConfigBase::set_deserialize(t_config_option_key opt_key, std::string str, bool append)
{
    if (!this->def->has(opt_key))
    {
...
void ConfigBase::set_deserialize_throw(t_config_option_key opt_key, std::string str, bool append)
{
...
// For example, return absolute infill extrusion width, either from an absolute value, or relative
// to the layer height.
double ConfigBase::get_abs_value(const t_config_option_key &opt_key) const
{
    const ConfigOption *opt = this->option(opt_key);
    if (const ConfigOptionFloatOrPercent *optv =
            dynamic_cast<const ConfigOptionFloatOrPercent *>(opt))
    {
...
double ConfigBase::get_abs_value(const t_config_option_key &opt_key, double ratio_over) const
{
    ...
    const ConfigOptionFloatOrPercent *opt =
        dynamic_cast<const ConfigOptionFloatOrPercent *>(this->option(opt_key));
    assert(opt != NULL);
...
bool ConfigBase::getBool(const t_config_option_key &opt_key) const
{
    return this->option_throw(opt_key)->getBool();
}

bool ConfigBase::getBool(const t_config_option_key &opt_key, bool default_value) const
{
    const ConfigOption *opt = this->option(opt_key);
    return opt == nullptr ? default_value : opt->getBool();
}

void ConfigBase::setBool(const t_config_option_key &opt_key, bool value)
{
    this->option(opt_key, true)->setBool(value);
}

double ConfigBase::getFloat(const t_config_option_key &opt_key) const
{
    return this->option_throw(opt_key)->getFloat();
}

double ConfigBase::getFloat(const t_config_option_key &opt_key, double default_value) const
{
    const ConfigOption *opt = this->option(opt_key);
    return opt == nullptr ? default_value : opt->getFloat();
}

void ConfigBase::setFloat(const t_config_option_key &opt_key, double value)
{
    this->option(opt_key, true)->setFloat(value);
}

int ConfigBase::getInt(const t_config_option_key &opt_key) const
{
    return this->option_throw(opt_key)->getInt();
}

int ConfigBase::getInt(const t_config_option_key &opt_key, int default_value) const
{
    const ConfigOption *opt = this->option(opt_key);
    return opt == nullptr ? default_value : opt->getInt();
}

void ConfigBase::setInt(const t_config_option_key &opt_key, int value)
{
    this->option(opt_key, true)->setInt(value);
}

std::string ConfigBase::getString(const t_config_option_key &opt_key) const
{
    return this->option_throw(opt_key)->getString();
}

std::string ConfigBase::getString(const t_config_option_key &opt_key,
...
void ConfigBase::setString(const t_config_option_key &opt_key, std::string value)
{
    this->option(opt_key, true)->setString(value);
}

std::vector<std::string> ConfigBase::getStrings(const t_config_option_key &opt_key) const
{
    return this->option_throw(opt_key)->getStrings();
}

std::vector<std::string> ConfigBase::getStrings(const t_config_option_key &opt_key,
...
void ConfigBase::setStrings(const t_config_option_key &opt_key, std::vector<std::string> value)
{
...
const ConfigOption *ConfigBase::option(const t_config_option_key &opt_key) const
{
    return const_cast<ConfigBase *>(this)->option(opt_key, false);
}

const ConfigOption *ConfigBase::option_throw(const t_config_option_key &opt_key) const
{
...
ConfigOption *ConfigBase::option(const t_config_option_key &opt_key, bool create)
{
    return this->optptr(opt_key, create);
}

ConfigOption *ConfigBase::option_throw(const t_config_option_key &opt_key, bool create)
{
...
void ConfigBase::save(const std::string &file) const
{
    ...
    for (t_config_option_keys::const_iterator opt_key = my_keys.begin(); opt_key != my_keys.end();
         ++opt_key)
        c << *opt_key << " = " << this->serialize(*opt_key) << endl;
    c.close();
}

void ConfigBase::validate() const
{
    for (auto &opt_key : this->keys())
    {
        // get option definition
...
void DynamicConfig::erase(const t_config_option_key &opt_key)
{
...
bool DynamicConfig::empty() const
{
...
{
    t_config_option_keys keys;
    for (t_optiondef_map::const_iterator it = this->def->options.begin();
         it != this->def->options.end(); ++it)
    {
...
bool ConfigOptionPoint::deserialize(std::string str, bool append)
{
    ...
    catch (boost::bad_lexical_cast &e)
    {
        Slic3r::Log::error("Config") << "Deserialisation error of [" << str << "]  : " << e.what()
                                     << " (expected 2D point)" << std::endl;
        return false;
    }
    return true;
};

bool ConfigOptionPoint3::deserialize(std::string str, bool append)
{
    ...
    {
        Slic3r::Log::error("Config") << "Deserialisation error of [" << str << "]  : " << e.what()
                                     << " (expected 3D point)" << std::endl;
        return false;
    }
    return true;
};

bool ConfigOptionPoints::deserialize(std::string str, bool append)
{
    if (!append)
        this->values.clear();
