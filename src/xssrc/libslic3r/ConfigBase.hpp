...
public:
    ...
    ConfigOptionException(const t_config_option_key _opt_key) : opt_key(_opt_key){};

    virtual const char *what() const noexcept
    {
        static std::string s("Exception with the option: ");
        s += this->opt_key;
...
public:
    virtual const char *what() const noexcept
    {
        static std::string s("Invalid value for option: ");
        s += this->opt_key;
        return s.c_str();
    }
};

/// Specialization of std::exception to indicate that an unsupported accessor was called on a config
/// option.
...
public:
    ...
    virtual void setStrings(std::vector<std::string> val)
    {
        throw BadOptionTypeException();
    };
...
/// Virtual base class, represents value of a vector valued option (bools, ints, floats, strings,
/// points)
...
public:
    ...
    ConfigOptionFloat *clone() const override
    {
        return new ConfigOptionFloat(this->value);
    };
    ...
    {
        return std::abs(value - other) < Slic3r::Geometry::epsilon;
    }
};
...
public:
    ...
    {
        return new ConfigOptionFloats(this->values);
    };

    std::string serialize() const
    {
        std::ostringstream ss;
        for (std::vector<double>::const_iterator it = this->values.begin();
...
public:
    ...
    {
        return new ConfigOptionInt(this->value);
    };
...
public:
    ...
    ConfigOptionInts *clone() const override
    {
        return new ConfigOptionInts(this->values);
    };

    std::string serialize() const override
    {
        std::ostringstream ss;
        for (std::vector<int>::const_iterator it = this->values.begin(); it != this->values.end();
             ++it)
        {
            if (it - this->values.begin() != 0)
                ss << ",";
...
public:
    ...
    ConfigOptionString *clone() const override
    {
        return new ConfigOptionString(this->value);
    };
...
public:
    ConfigOptionStrings(){};
    ConfigOptionStrings(const std::vector<std::string> _values) :
        ConfigOptionVector<std::string>(_values){};
    ConfigOptionStrings *clone() const
    {
        return new ConfigOptionStrings(this->values);
    };
...
public:
    ...
    {
        return new ConfigOptionPercent(this->value);
    };
...
public:
    ...
    ConfigOptionFloatOrPercent *clone() const override
    {
        return new ConfigOptionFloatOrPercent(this->value, this->percent);
    };
    ...
    {
        const ConfigOptionFloatOrPercent *other =
            dynamic_cast<const ConfigOptionFloatOrPercent *>(&option);
        if (other != NULL)
        {
            this->value = other->value;
...
public:
    ...
    ConfigOptionPoint *clone() const
    {
        return new ConfigOptionPoint(this->value);
    };
...
public:
    ...
    ConfigOptionPoint3 *clone() const
    {
        return new ConfigOptionPoint3(this->value);
    };
...
public:
    ...
    ConfigOptionPoints *clone() const
    {
        return new ConfigOptionPoints(this->values);
    };
...
public:
    ...
    {
        return new ConfigOptionBool(this->value);
    };
...
public:
    ...
    {
        return new ConfigOptionBools(this->values);
    };

    std::string serialize() const
    {
        std::ostringstream ss;
        for (std::vector<bool>::const_iterator it = this->values.begin(); it != this->values.end();
             ++it)
        {
            if (it - this->values.begin() != 0)
                ss << ",";
...
public:
    ...
    {
        return new ConfigOptionEnum<T>(this->value);
    };

    std::string serialize() const
    {
        t_config_enum_values enum_keys_map = ConfigOptionEnum<T>::get_enum_values();
        for (t_config_enum_values::iterator it = enum_keys_map.begin(); it != enum_keys_map.end();
             ++it)
        {
            if (it->second == static_cast<int>(this->value))
                return it->first;
        }
        return "";
    };
...
///
/// We use this one in DynamicConfig objects when creating a config value object for
/// ConfigOptionType == coEnum. In the StaticConfig, it is better to use the specialized
/// ConfigOptionEnum<T> containers.
...
public:
    ...
    std::string serialize() const
    {
        for (t_config_enum_values::const_iterator it = this->keys_map->begin();
             it != this->keys_map->end(); ++it)
        {
...
enum ConfigOptionType
{
    ...
    coPoint,
    /// vector of 2d points. Currently used for the definition of the print bed and for the extruder
    /// offsets.
    coPoints,
    coPoint3,
...
public:
    ...
    ///
    /// Usually empty.
    /// Special values - "i_enum_open", "f_enum_open" to provide combo box for int or float
    /// selection, "select_open" - to open a selection dialog (currently only a serial port
    /// selection).
    std::string gui_type;
    /// The flags may be combined.
    /// "show_value" - even if enum_values / enum_labels are set, still display the value, not the
    /// enum label. "align_label_right" - align label to right
    std::string gui_flags;
    /// Label of the GUI input field.
    /// In case the GUI input fields are grouped in some views, the label defines a short label of a
    /// grouped value, while full_label contains a label of a stand-alone field. The full label is
    /// shown, when adding an override parameter for an object or a modified object.
    std::string label;
    std::string full_label;
    /// Category of a configuration field, from the GUI perspective.
    /// One of: "Layers and Perimeters", "Infill", "Support material", "Speed", "Extruders",
    ...
    /// Mostly used for enums (when type == coEnum), but may be used for ints resp. floats, if
    /// gui_type is set to "i_enum_open" resp. "f_enum_open".
    ...
    ConfigOptionDef() :
        type(coNone), default_value(nullptr), multiline(false), full_width(false), readonly(false),
...
// i The definition does not carry an actual value of the config option, only its constant default
// value. i t_config_option_key is std::string
typedef std::map<t_config_option_key, ConfigOptionDef> t_optiondef_map;

/// Definition of configuration values for the purpose of GUI presentation, editing, value mapping
/// and config file handling. The configuration definition is static: It does not carry the actual
/// configuration values, but it carries the defaults of the configuration values.
class ConfigDef
{
public:
...
{
public:
    /// Definition of configuration values for the purpose of GUI presentation, editing, value
    /// mapping and config file handling. The configuration definition is static: It does not carry
    /// the actual configuration values, but it carries the defaults of the configuration values.
    ...
    template <class T>
    T *opt_throw(const t_config_option_key &opt_key, bool create = false)
    {
    ...
    template <class T>
    const T *opt_throw(const t_config_option_key &opt_key) const
    {
    ...
    void apply_only(const ConfigBase &other, const t_config_option_keys &opt_keys,
    ...
    /// @param ignore_nonexistant if true, don't throw an exception if the key is not known to
    /// Slic3r
    /// @default nonexistent Set the configuration option to its default if it is not found in
    /// other.
    void apply_only(const ConfigBase &other, const t_config_option_keys &opt_keys,
                    bool ignore_nonexistent = false, bool default_nonexistent = false);
    ...
    std::vector<std::string> getStrings(const t_config_option_key &opt_key,
...
/// In Slic3r, the static configuration stores are during the slicing / g-code generation for
/// efficiency reasons, because the configuration values could be accessed directly.
class StaticConfig : public virtual ConfigBase
{
public:
    StaticConfig() : ConfigBase(){};
    /// Gets list of config option names for each config option of this->def, which has a static
    /// counter-part defined by the derived object and which could be resolved by this->optptr(key)
    /// call.
    t_config_option_keys keys() const;
