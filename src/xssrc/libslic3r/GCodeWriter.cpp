...
std::string GCodeWriter::notes()
{
    ...
    for (auto i = this->config.filament_notes.values.cbegin();
         i != this->config.filament_notes.values.cend(); i++)
    {
        if (i->size() > 0)
        {
    ...
    if (this->config.printer_notes.getString().size() > 0)
    {
        ...
        std::vector<std::string> temp_line =
            split_at_regex(this->config.printer_notes.getString(), "\n");
        for (auto j = temp_line.cbegin(); j != temp_line.cend(); j++)
        {
            gcode << "; " << *j << "\n";
        }
...
std::string GCodeWriter::preamble()
{
    ...
    if (FLAVOR_IS(gcfRepRap) || FLAVOR_IS(gcfTeacup) || FLAVOR_IS(gcfRepetier)
...
std::string GCodeWriter::set_temperature(unsigned int temperature, bool wait, int tool) const
{
    ...
    if (wait && FLAVOR_IS_NOT(gcfTeacup) && FLAVOR_IS_NOT(gcfMakerWare)
        && FLAVOR_IS_NOT(gcfSailfish))
    {
        code = "M109";
    ...
    gcode << temperature;
    if (tool != -1
        && (this->multiple_extruders || FLAVOR_IS(gcfMakerWare) || FLAVOR_IS(gcfSailfish)))
    {
        gcode << " T" << tool;
    }
    gcode << " ; " << comment << "\n";
...
bool GCodeWriter::need_toolchange(unsigned int extruder_id) const
{
...
{
    if (!this->need_toolchange(extruder_id))
        return "";
...
std::string GCodeWriter::set_speed(double F, const std::string &comment,
...
std::string GCodeWriter::travel_to_xy(const Pointf &point, const std::string &comment)
{
    ...
    std::ostringstream gcode;
    gcode << "G1 X" << XYZF_NUM(point.x) << " Y" << XYZF_NUM(point.y) << " F"
          << XYZF_NUM(this->config.travel_speed.value * 60.0);
    COMMENT(comment);
    gcode << "\n";
    return gcode.str();
}
...
std::string GCodeWriter::travel_to_xyz(const Pointf3 &point, const std::string &comment)
{
    ...
    std::ostringstream gcode;
    gcode << "G1 X" << XYZF_NUM(point.x) << " Y" << XYZF_NUM(point.y) << " Z" << XYZF_NUM(point.z)
...
std::string GCodeWriter::_travel_to_z(double z, const std::string &comment)
{
    ...
    std::ostringstream gcode;
    gcode << "G1 Z" << XYZF_NUM(z) << " F" << XYZF_NUM(this->config.travel_speed.value * 60.0);
    COMMENT(comment);
    gcode << "\n";
    return gcode.str();
}
...
std::string GCodeWriter::extrude_to_xy(const Pointf &point, double dE, const std::string &comment)
{
    ...
    std::ostringstream gcode;
    gcode << "G1 X" << XYZF_NUM(point.x) << " Y" << XYZF_NUM(point.y) << " "
...
std::string GCodeWriter::retract()
{
    return this->_retract(this->_extruder->retract_length(),
                          this->_extruder->retract_restart_extra(), "retract");
}

std::string GCodeWriter::retract_for_toolchange()
{
    return this->_retract(this->_extruder->retract_length_toolchange(),
                          this->_extruder->retract_restart_extra_toolchange(),
                          "retract for toolchange", true);
}
                                  ...
                                  bool long_retract)
{
    ...
    if (dE != 0)
    {
        ...
        {
            gcode << "G1 " << this->_extrusion_axis << E_NUM(this->_extruder->E) << " F"
                  << this->_extruder->retract_speed_mm_min;
        }
    ...
    if (dE != 0)
    {
        if (this->config.use_firmware_retraction)
        {
            if (FLAVOR_IS(gcfMachinekit))
                ...
                gcode << "G11";
            if (this->config.gcode_comments)
                gcode << " ; unretract extruder " << this->_extruder->id;
            gcode << "\n";
            gcode << this->reset_e();
        }
        else
