...
void FillPlanePath::_fill_surface_single(unsigned int thickness_layers,
                                         const direction_t &direction, ExPolygon &expolygon,
                                         Polylines *polylines_out)
{
    ...
    BoundingBox bounding_box =
        this->bounding_box.defined ? this->bounding_box : expolygon.contour.bounding_box();
    bounding_box = bounding_box.rotated(-direction.first);

    const Point shift = this->_centered() ? bounding_box.center() : bounding_box.min;
    expolygon.translate(-shift.x, -shift.y);
    bounding_box.translate(-shift.x, -shift.y);

    const Pointfs pts =
        this->_generate(coord_t(ceil(coordf_t(bounding_box.min.x) / distance_between_lines)),
    ...
    if (pts.size() >= 2)
    {
        ...
        {
            polyline.points.push_back(Point(coord_t(floor(it->x * distance_between_lines + 0.5)),
                                            coord_t(floor(it->y * distance_between_lines + 0.5))));
        }
        //      polylines = intersection_pl(polylines_src, offset((Polygons)expolygon,
        //      scale_(0.02)));
...
Pointfs FillArchimedeanChords::_generate(coord_t min_x, coord_t min_y, coord_t max_x, coord_t max_y)
{
    // Radius to achieve.
    const coordf_t rmax =
        std::sqrt(coordf_t(max_x) * coordf_t(max_x) + coordf_t(max_y) * coordf_t(max_y))
            * std::sqrt(2.)
        + 1.5;
