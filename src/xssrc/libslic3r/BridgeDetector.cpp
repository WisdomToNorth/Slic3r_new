...
BridgeDetector::BridgeDetector(const ExPolygon &_expolygon,
                               const ExPolygonCollection &_lower_slices, coord_t _extrusion_width) :
    expolygon(_expolygon),
    extrusion_width(_extrusion_width), resolution(PI / 36.0), angle(-1)
{
    ...
    // negatives)
    this->lower_slices.expolygons =
        offset2_ex(_lower_slices, +this->extrusion_width / 2, -this->extrusion_width / 2);
    ...
    // safety offset required to avoid Clipper from detecting empty intersection while Boost
    // actually found some edges
...
bool BridgeDetector::detect_angle()
{
    ...
    if (this->_edges.empty() || this->_anchors.empty())
        return false;
    ...
    for (BridgeDirection &candidate : candidates)
    {
        ...
        for (const Line &line : clipped_lines)
        {
            ...
            candidate.coverage += Slic3r::Geometry::area(
                intersection(my_clip_area, offset((Polyline)line, +this->extrusion_width / 2)));
        }
    ...
    // TODO: There are two options here - within width of the angle with most coverage, or within
    // width of the currently preferred?
    size_t i_best = 0;
    for (size_t i = 1;
         i < candidates.size()
         && candidates[i_best].coverage - candidates[i].coverage < this->extrusion_width;
         ++i)
...
Polygons BridgeDetector::coverage() const
{
    if (this->angle == -1)
        return Polygons();
...
Polylines BridgeDetector::unsupported_edges(double angle) const
{
    ...
    Polylines _unsupported =
        diff_pl(bridge_edges, offset(this->lower_slices, +this->extrusion_width));
