...
std::string PlaceholderParser::process(std::string str) const
{
    ...
    // without a regex)
    for (t_strstrs_map::const_iterator it = this->_multiple.begin(); it != this->_multiple.end();
         ++it)
    {
...
bool PlaceholderParser::find_and_replace(std::string &source, std::string const &find,
