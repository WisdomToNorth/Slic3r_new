...
void LayerRegion::make_fill()
{
    ...
    // without any angle (shouldn't this logic be moved to process_external_surfaces()?)
    {
        ...
        for (Surfaces::const_iterator it = this->fill_surfaces.surfaces.begin();
             it != this->fill_surfaces.surfaces.end(); ++it)
            if (it->is_bridge() && it->bridge_angle >= 0)
                append_to(polygons_bridged, (Polygons)*it);

        // group surfaces by distinct properties (equal surface_type, thickness, thickness_layers,
        // bridge_angle) group is of type SurfaceCollection
        ...
        // merge compatible solid groups (we can generate continuous infill for them)
        {
            ...
            for (size_t i = 0; i < groups.size(); ++i)
            {
                ...
                if (!surface.is_solid() || surface.is_bridge())
                    continue;

                group_attrib[i].is_solid = true;
                group_attrib[i].fw =
                    (surface.is_top()) ? top_solid_infill_flow.width : solid_infill_flow.width;
                group_attrib[i].pattern =
                    surface.is_top()    ? this->region()->config.top_infill_pattern.value :
        ...
        // Give priority to oriented bridges. Process the bridges in the first round, the rest of
        // the surfaces in the 2nd round.
        for (size_t round = 0; round < 2; ++round)
        {
            for (std::vector<SurfacesConstPtr>::const_iterator it_group = groups.begin();
                 it_group != groups.end(); ++it_group)
            {
                const SurfacesConstPtr &group = *it_group;
                const bool is_oriented_bridge =
                    group.front()->is_bridge() && group.front()->bridge_angle >= 0;
                if (is_oriented_bridge != (round == 0))
                    continue;

                // Make a union of polygons defining the infiill regions of a group, use a safety
                // offset.
                Polygons union_p = union_(to_polygons(group), true);

                // Subtract surfaces having a defined bridge_angle from any other, use a safety
                // offset.
                ...
                surfaces.append(diff_ex(union_p, to_polygons(surfaces), true),
    ...
    // any void neighbors
    {
        coord_t distance_between_surfaces =
            std::max(std::max(infill_flow.scaled_spacing(), solid_infill_flow.scaled_spacing()),
                     top_solid_infill_flow.scaled_spacing());

        Polygons surfaces_polygons = (Polygons)surfaces;
        Polygons collapsed = diff(surfaces_polygons,
                                  offset2(surfaces_polygons, -distance_between_surfaces / 2,
        ...
        surfaces.append(
            intersection_ex(offset(collapsed, distance_between_surfaces), to_subtract, true),
            (stInternal | stSolid));
    }
         ...
         surface_it != surfaces.surfaces.end(); ++surface_it)
    {
        ...
        if (surface.is_solid())
        {
            ...
            fill_pattern = (surface.is_top()) ? this->region()->config.top_infill_pattern.value :
                           (surface.is_bottom() && !is_bridge) ?
                                                this->region()->config.bottom_infill_pattern.value :
                                                ipRectilinear;
        }
        else if (density <= 0)
            continue;
        ...
        Flow flow = this->region()->flow(role, h,
        ...
        if (!surface.is_solid() && !is_bridge)
        {
            ...
            Flow internal_flow = this->region()->flow(
                frInfill,
                h, // use the calculated surface thickness here for internal infill instead of the
                   // layer height to account for infill_every_layers
                false, // no bridge
        ...
        f->endpoints_overlap = scale_(this->region()->config.get_abs_value(
            "infill_overlap", (unscale(perimeter_spacing) + (f->min_spacing)) / 2));
        f->layer_id = this->layer()->id();
        f->z = this->layer()->print_z;
        f->angle = Geometry::deg2rad(this->region()->config.fill_angle.value);

        // Maximum length of the perimeter segment linking two infill lines.
        f->link_max_length = (!is_bridge && density > 80) ? scale_(3 * f->min_spacing) : 0;
        ...
        */
        ...
        {
            flow = Flow::new_from_spacing(f->spacing(), flow.nozzle_diameter, h,
    ...
    // thin_fills are of C++ Slic3r::ExtrusionEntityCollection, perl type
    // Slic3r::ExtrusionPath::Collection Unpacks the collection, creates multiple collections per
    // path so that they will be individually included in the nearest neighbor search. The path type
    // could be ExtrusionPath, ExtrusionLoop or ExtrusionEntityCollection.
    for (ExtrusionEntitiesPtr::const_iterator thin_fill = this->thin_fills.entities.begin();
         thin_fill != this->thin_fills.entities.end(); ++thin_fill)
    {
        ExtrusionEntityCollection *coll = new ExtrusionEntityCollection();
        this->fills.entities.push_back(coll);
