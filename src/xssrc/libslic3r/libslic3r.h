...
inline constexpr coord_t scale_(const coordf_t &val)
{
    return val / SCALING_FACTOR;
}
inline constexpr coordf_t unscale(const coord_t &val)
{
    return val * SCALING_FACTOR;
}
...
// RESOLUTION, SCALED_RESOLUTION: Used as an error threshold for a Douglas-Peucker polyline
...
template <class T>
void _parallelize_do(std::queue<T> *queue, std::mutex *queue_mutex, std::function<void(T)> func)
{
...
template <class T>
void parallelize(std::queue<T> queue, std::function<void(T)> func,
                 int threads_count = std::thread::hardware_concurrency())
{
    if (threads_count == 0)
        threads_count = 2;
    std::mutex queue_mutex;
    std::thread_group workers;

    for (int i = 0; i < std::min(threads_count, (int)queue.size()); i++)
        workers.add_thread(new std::thread(&_parallelize_do<T>, &queue, &queue_mutex, func));
    workers.join_all();
}

template <class T>
void parallelize(T start, T end, std::function<void(T)> func,
                 int threads_count = std::thread::hardware_concurrency())
{
    std::queue<T> queue;
