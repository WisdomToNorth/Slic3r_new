...
std::string evaluate(const std::string &expression_string)
{
    ...
    std::cerr << __FILE__ << ":" << __LINE__ << " "
              << "Evaluating expression: " << expression_string << std::endl;
#endif
...
#if SLIC3R_DEBUG
        std::cerr << __FILE__ << ":" << __LINE__ << " "
                  << "Failed to parse: " << expression_string.c_str() << std::endl;
#endif
...
/// Parse an expression and return a string. We assume that PlaceholderParser has expanded all
/// variables.
std::string expression(const std::string &input, const int depth)
{
...
#ifdef SLIC3R_DEBUG
    std::cerr << __FILE__ << ":" << __LINE__ << " "
              << "depth " << depth << " input str: " << input << std::endl;
#endif
    if (open_bracket == 0 && depth > 0)
    { // no subexpressions, resolve operators.
    ...
    while (open_bracket > 0)
    {
...
#ifdef SLIC3R_DEBUG
        std::cerr << __FILE__ << ":" << __LINE__ << " "
                  << "depth " << depth << " loop " << i << " pos: " << pos << std::endl;
#endif
        ...
        std::string retval =
            expression(buffer.substr(pos + shift_if, (end_pos - (pos + shift_if))), depth + 1);

#ifdef SLIC3R_DEBUG
        if (is_conditional)
        {
            std::cerr << __FILE__ << ":" << __LINE__ << " "
                      << "depth " << depth << " loop " << i << " return: '" << retval << "'"
                      << std::endl;
        }
#endif
...
#ifdef SLIC3R_DEBUG
        std::cerr << __FILE__ << ":" << __LINE__ << " "
                  << "depth: " << depth << " Result from loop " << i << ": " << buffer << std::endl;
#endif
    ...
    // {if that resolves to false/0 signifies dropping everything up to the next newline from the
    // input buffer. Also remove the result of the {if itself.
    return buffer;
}
