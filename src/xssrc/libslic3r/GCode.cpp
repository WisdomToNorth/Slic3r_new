...
AvoidCrossingPerimeters::AvoidCrossingPerimeters() :
    use_external_mp(false), use_external_mp_once(false), disable_once(true), _external_mp(NULL),
    _layer_mp(NULL)
{
}
...
void AvoidCrossingPerimeters::init_external_mp(const ExPolygons &islands)
{
    delete this->_external_mp;
...
void AvoidCrossingPerimeters::init_layer_mp(const ExPolygons &islands)
{
    delete this->_layer_mp;
...
std::string Wipe::wipe(GCode &gcodegen, bool toolchange)
{
    ...
    double length = toolchange ? gcodegen.writer.extruder()->retract_length_toolchange() :
    ...
    if (length > 0)
    {
        ...
        double wipe_dist =
            scale_(length / gcodegen.writer.extruder()->retract_speed() * wipe_speed);
        ...
        for (Lines::const_iterator line = lines.begin(); line != lines.end(); ++line)
        {
            ...
            /*  Reduce retraction length a bit to avoid effective retraction speed to be greater
               than the configured one due to rounding (TODO: test and/or better math for this)  */
            double dE = length * (segment_length / wipe_dist) * 0.95;
            gcode += gcodegen.writer.set_speed(wipe_speed * 60, "",
                                               gcodegen.enable_cooling_markers ? ";_WIPE" : "");
            gcode += gcodegen.writer.extrude_to_xy(gcodegen.point_to_gcode(line->b), -dE,
                                                   "wipe and retract");
            retracted += dE;
        }
...
GCode::GCode() :
    placeholder_parser(NULL), enable_loop_clipping(true), enable_cooling_markers(false),
    layer_count(0), layer_index(-1), layer(NULL), first_layer(false), elapsed_time(0.0),
    elapsed_time_bridges(0.0), elapsed_time_external(0.0), volumetric_speed(0),
    _last_pos_defined(false)
{
}

const Point &GCode::last_pos() const
{
...
bool GCode::last_pos_defined() const
{
...
void GCode::set_origin(const Pointf &pointf)
{
    ...
    const Point translate(scale_(this->origin.x - pointf.x), scale_(this->origin.y - pointf.y));
    this->_last_pos.translate(translate);
    this->wipe.path.translate(translate);
    this->origin = pointf;
}

std::string GCode::notes()
{
...
std::string GCode::extrude(ExtrusionLoop loop, std::string description, double speed)
{
    ...
    else if (seam_position == spNearest || seam_position == spAligned || seam_position == spRear)
    {
        ...
        else if (!candidates.empty())
        {
            ...
            last_pos.nearest_point(candidates, &point);
            if (!loop.split_at_vertex(point))
                loop.split_at(point); // see note above
        }
        else
        {
    ...
    if (paths.front().is_perimeter() && !loop.has(erOverhangPerimeter)
        && loop.length() <= SMALL_PERIMETER_LENGTH && speed == -1)
    {
    ...
    if (paths.back().role == erExternalPerimeter && this->layer != NULL
        && this->config.perimeters > 1)
    {
        // detect angle between last and first segment
        // the side depends on the original winding order of the polygon (left for contours, right
        // for holes)
        ...
        Line first_segment(paths.front().polyline.points[0], paths.front().polyline.points[1]);
        const double distance =
            std::min((double)scale_(EXTRUDER_CONFIG(nozzle_diameter)), first_segment.length());
        Point point = first_segment.point_at(distance);
        point.rotate(angle, first_segment.a);

        // generate the travel move
        gcode +=
            this->writer.travel_to_xy(this->point_to_gcode(point), "move inwards before travel");
    }
...
std::string GCode::extrude(const ExtrusionPath &path, std::string description, double speed)
{
...
std::string GCode::_extrude(ExtrusionPath path, std::string description, double speed)
{
    ...
    {
        gcode += this->travel_to(path.first_point(), path.role,
    ...
    if (this->writer.extrusion_axis().empty())
        e_per_mm = 0;
    ...
    if (this->config.max_volumetric_speed.value > 0)
    {
        ...
        speed = std::min(speed, this->config.max_volumetric_speed.value / path.mm3_per_mm);
    }
    ...
    double path_length = 0;
    {
        ...
        for (Lines::const_iterator line = lines.begin(); line != lines.end(); ++line)
        {
            ...
            this->_cog.x += (this->point_to_gcode(line->a).x + this->point_to_gcode(line->b).x) / 2
                            * line_length;
            this->_cog.y += (this->point_to_gcode(line->a).y + this->point_to_gcode(line->b).y) / 2
                            * line_length;
            this->_cog.z += this->writer.get_position().z * line_length;
            this->_extrusion_length += line_length;

            gcode += this->writer.extrude_to_xy(this->point_to_gcode(line->b),
    ...
    if (this->config.cooling)
    {
        ...
        this->elapsed_time += t;
        if (path.is_bridge())
            this->elapsed_time_bridges += t;
        if (path.role == erExternalPerimeter)
            this->elapsed_time_external += t;
    }
...
std::string GCode::travel_to(const Point &point, ExtrusionRole role, std::string comment)
{
    ...
    if (needs_retraction && this->config.avoid_crossing_perimeters
...
Pointf GCode::point_to_gcode(const Point &point)
{
    Pointf extruder_offset = EXTRUDER_CONFIG(extruder_offset);
    return Pointf(unscale(point.x) + this->origin.x - extruder_offset.x,
                  unscale(point.y) + this->origin.y - extruder_offset.y);
}
} // namespace Slic3r

Pointf3 GCode::get_cog()
{
    Pointf3 result_cog;
