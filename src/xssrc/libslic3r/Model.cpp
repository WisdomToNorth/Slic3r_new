...
{
    // copy materials
    for (ModelMaterialMap::const_iterator i = other.materials.begin(); i != other.materials.end();
         ++i)
...
Model Model::read_from_file(std::string input_file)
{
...
ModelMaterial *Model::get_material(t_model_material_id material_id)
{
    ModelMaterialMap::iterator i = this->materials.find(material_id);
    if (i == this->materials.end())
    {
...
bool Model::has_objects_with_no_instances() const
{
    for (ModelObjectPtrs::const_iterator i = this->objects.begin(); i != this->objects.end(); ++i)
    {
...
void Model::center_instances_around_point(const Pointf &point)
{
    ...
    for (ModelObjectPtrs::const_iterator o = this->objects.begin(); o != this->objects.end(); ++o)
    {
        for (ModelInstancePtrs::const_iterator i = (*o)->instances.begin();
             i != (*o)->instances.end(); ++i)
        {
...
// flattens everything to a single mesh
TriangleMesh Model::raw_mesh() const
{
    TriangleMesh mesh;
    for (ModelObjectPtrs::const_iterator o = this->objects.begin(); o != this->objects.end(); ++o)
    {
...
bool Model::_arrange(const Pointfs &sizes, coordf_t dist, const BoundingBoxf *bb,
                     Pointfs &out) const
{
    // we supply unscaled data to arrange()
    bool result =
        Slic3r::Geometry::arrange(sizes.size(),            // number of parts
...
void Model::duplicate_objects_grid(size_t x, size_t y, coordf_t dist)
{
    if (this->objects.size() > 1)
        throw std::runtime_error("Grid duplication is not supported with multiple objects");
...
void ModelMaterial::apply(const t_model_material_attributes &attributes)
{
    this->attributes.insert(attributes.begin(), attributes.end());
}

ModelObject::ModelObject(Model *model) : part_number(-1), _bounding_box_valid(false), model(model)
{
}
    ...
    _bounding_box(other._bounding_box), _bounding_box_valid(other._bounding_box_valid), model(model)
{
    if (copy_volumes)
    {
        ...
        for (ModelVolumePtrs::const_iterator i = other.volumes.begin(); i != other.volumes.end();
             ++i)
            this->add_volume(**i);
    }

    this->instances.reserve(other.instances.size());
    for (ModelInstancePtrs::const_iterator i = other.instances.begin(); i != other.instances.end();
         ++i)
        this->add_instance(**i);
}
...
void ModelObject::invalidate_bounding_box()
{
    this->_bounding_box_valid = false;
}

void ModelObject::update_bounding_box()
{
    ...
    BoundingBoxf3 bb;
    for (ModelInstancePtrs::const_iterator i = this->instances.begin(); i != this->instances.end();
         ++i)
...
Pointf3 ModelObject::origin_translation() const
{
...
TriangleMesh ModelObject::mesh() const
{
         ...
         ++i)
    {
        ...
        for (ModelVolumePtrs::const_iterator v = this->volumes.begin(); v != this->volumes.end();
             ++v)
        {
...
BoundingBoxf3 ModelObject::raw_bounding_box() const
{
    ...
    if (this->instances.empty())
        CONFESS("Can't call raw_bounding_box() with no instances");
...
BoundingBoxf3 ModelObject::instance_bounding_box(size_t instance_idx) const
{
    BoundingBoxf3 bb;
    if (this->instances.size() <= instance_idx)
        CONFESS("Can't call instance_bounding_box(index) with insufficient amount of instances");
...
void Model::align_instances_to_origin()
{
    BoundingBoxf3 bb = this->bounding_box();
...
void ModelObject::align_to_ground()
{
    BoundingBoxf3 bb;
...
void ModelObject::center_around_origin()
{
    ...
    if (!this->instances.empty())
    {
        for (ModelInstancePtrs::const_iterator i = this->instances.begin();
...
void ModelObject::scale(double factor)
{
...
void ModelObject::scale(const Pointf3 &versor)
{
    ...
    TransformationMatrix trafo = TransformationMatrix::multiply(
        TransformationMatrix::mat_scale(versor.x, versor.y, versor.z), center_trafo);
...
void ModelObject::scale_to_fit(const Sizef3 &size)
{
    Sizef3 orig_size = this->bounding_box().size();
    double factor = fminf(size.x / orig_size.x, fminf(size.y / orig_size.y, size.z / orig_size.z));
    this->scale(factor);
}

void ModelObject::rotate(double angle, const Axis &axis)
{
    if (angle == 0)
        return;
    ...
    TransformationMatrix center_trafo = this->get_trafo_to_center();
    TransformationMatrix trafo = TransformationMatrix::multiply(
        TransformationMatrix::mat_rotation(angle, axis), center_trafo);
    trafo.applyLeft(center_trafo.inverse());
...
void ModelObject::rotate(double angle, const Vectorf3 &axis)
{
    if (angle == 0)
        return;
...
void ModelObject::rotate(const Vectorf3 &origin, const Vectorf3 &target)
{
    TransformationMatrix center_trafo = this->get_trafo_to_center();
    TransformationMatrix trafo = TransformationMatrix::multiply(
        TransformationMatrix::mat_rotation(origin, target), center_trafo);
    trafo.applyLeft(center_trafo.inverse());
...
void ModelObject::mirror(const Axis &axis)
{
    ...
    TransformationMatrix trafo =
        TransformationMatrix::multiply(TransformationMatrix::mat_mirror(axis), center_trafo);
    trafo.applyLeft(center_trafo.inverse());
...
void ModelObject::apply_transformation(const TransformationMatrix &trafo)
{
...
void ModelObject::transform_by_instance(ModelInstance instance, bool dont_translate)
{
...
void ModelObject::cut(Axis axis, coordf_t z, Model *model) const
{
...
void ModelObject::split(ModelObjectPtrs *new_objects)
{
    ...
    for (TriangleMeshPtrs::iterator mesh = meshptrs.begin(); mesh != meshptrs.end(); ++mesh)
    {
        ...
        new_object->part_number =
            this->part_number; // According to 3mf part number should be given to the split parts.
...
ModelVolume::ModelVolume(ModelObject *object, const ModelVolume &other) :
    name(other.name), mesh(other.mesh), trafo(other.trafo), config(other.config),
    input_file(other.input_file), input_file_obj_idx(other.input_file_obj_idx),
    input_file_vol_idx(other.input_file_vol_idx), modifier(other.modifier), object(object)
{
...
void ModelVolume::swap(ModelVolume &other)
{
...
TriangleMesh ModelVolume::get_transformed_mesh(TransformationMatrix const &trafo) const
{
    return this->mesh.get_transformed_mesh(trafo);
}

BoundingBoxf3 ModelVolume::get_transformed_bounding_box(TransformationMatrix const &trafo) const
{
    return this->mesh.get_transformed_bounding_box(trafo);
}

BoundingBoxf3 ModelVolume::bounding_box() const
{
...
t_model_material_id ModelVolume::material_id() const
{
...
ModelInstance::ModelInstance(ModelObject *object) : rotation(0), scaling_factor(1), object(object)
{
}
...
ModelInstance::ModelInstance(ModelObject *object, const ModelInstance &other) :
    rotation(other.rotation), scaling_factor(other.scaling_factor), offset(other.offset),
    additional_trafo(other.additional_trafo), object(object)
{
}
...
void ModelInstance::swap(ModelInstance &other)
{
...
void ModelInstance::set_complete_trafo(TransformationMatrix const &trafo)
{
    ...
    TransformationMatrix rotmat =
        trafo.multiplyLeft(TransformationMatrix::mat_scale(1 / sx, 1 / sy, 1 / sz));
    ...
    this->additional_trafo =
        TransformationMatrix::multiply(this->get_trafo_matrix().inverse(), trafo);
}
...
BoundingBoxf3 ModelInstance::transform_bounding_box(const BoundingBoxf3 &bbox,
                                                    bool dont_translate) const
{
    TransformationMatrix trafo = this->get_trafo_matrix(dont_translate);
    ...
    Pointf3 pts[8] = {
        Pointf3(Poi_min.x, Poi_min.y, Poi_min.z), Pointf3(Poi_min.x, Poi_min.y, Poi_max.z),
        Pointf3(Poi_min.x, Poi_max.y, Poi_min.z), Pointf3(Poi_min.x, Poi_max.y, Poi_max.z),
        Pointf3(Poi_max.x, Poi_min.y, Poi_min.z), Pointf3(Poi_max.x, Poi_min.y, Poi_max.z),
        Pointf3(Poi_max.x, Poi_max.y, Poi_min.z), Pointf3(Poi_max.x, Poi_max.y, Poi_max.z)};
    BoundingBoxf3 out;
    for (int i = 0; i < 8; ++i)
    {
