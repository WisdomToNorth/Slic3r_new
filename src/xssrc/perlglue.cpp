...
SV *ConfigOption_to_SV(const ConfigOption &opt, const ConfigOptionDef &def)
{
    ...
    else if (def.type == coFloats)
    {
        ...
        for (std::vector<double>::const_iterator it = optv->values.begin();
             it != optv->values.end(); ++it)
    ...
    else if (def.type == coInts)
    {
        ...
        AV *av = newAV();
        av_fill(av, optv->values.size() - 1);
        for (std::vector<int>::const_iterator it = optv->values.begin(); it != optv->values.end();
             ++it)
    ...
    else if (def.type == coStrings)
    {
        ...
        AV *av = newAV();
        av_fill(av, optv->values.size() - 1);
        for (std::vector<std::string>::const_iterator it = optv->values.begin();
             it != optv->values.end(); ++it)
    ...
    else if (def.type == coBools)
    {
        ...
        AV *av = newAV();
        av_fill(av, optv->values.size() - 1);
        for (std::vector<bool>::const_iterator it = optv->values.begin(); it != optv->values.end();
             ++it)
...
bool ConfigBase__set(ConfigBase *THIS, const t_config_option_key &opt_key, SV *value)
{
    ...
    else if (def.type == coFloats)
    {
        ...
        for (size_t i = 0; i < len; i++)
        {
            ...
            if (elem == NULL || !looks_like_number(*elem))
                return false;
    ...
    else if (def.type == coPoints)
    {
        ...
        for (size_t i = 0; i < len; i++)
        {
            SV **elem = av_fetch(av, i, 0);
            Pointf point;
            if (elem == NULL || !from_SV_check(*elem, &point))
                return false;
...
void ConfigBase__set_ifndef(ConfigBase *THIS, const t_config_option_key &opt_key, SV *value,
                            bool deserialize)
{
    if (!THIS->has(opt_key))
    {
...
bool StaticConfig__set(StaticConfig *THIS, const t_config_option_key &opt_key, SV *value)
{
    ...
    {
        for (std::vector<t_config_option_key>::const_iterator it = optdef.shortcut.begin();
             it != optdef.shortcut.end(); ++it)
        {
            if (!StaticConfig__set(THIS, *it, value))
                return false;
        }
        return true;
    }
...
void from_SV_check(SV *expoly_sv, ExPolygon *expolygon)
{
    ...
    {
        if (!sv_isa(expoly_sv, perl_class_name(expolygon))
            && !sv_isa(expoly_sv, perl_class_name_ref(expolygon)))
...
{
    if (sv_isobject(poly_sv) && !sv_isa(poly_sv, perl_class_name(THIS))
        && !sv_isa(poly_sv, perl_class_name_ref(THIS)))
        CONFESS("Not a valid %s object", perl_class_name(THIS));
...
void from_SV_check(SV *point_sv, Point *point)
{
    ...
    {
        if (!sv_isa(point_sv, perl_class_name(point))
            && !sv_isa(point_sv, perl_class_name_ref(point)))
            CONFESS("Not a valid %s object (got %s)", perl_class_name(point),
                    HvNAME(SvSTASH(SvRV(point_sv))));
...
bool from_SV(SV *point_sv, Pointf *point)
{
    ...
    if (!looks_like_number(sv_x) || !looks_like_number(sv_y))
        return false;
...
void from_SV_check(SV *point3_sv, Point3 *point)
{
    ...
    {
        if (!sv_isa(point3_sv, perl_class_name(point))
            && !sv_isa(point3_sv, perl_class_name_ref(point)))
            CONFESS("Not a valid %s object (got %s)", perl_class_name(point),
                    HvNAME(SvSTASH(SvRV(point3_sv))));
...
bool from_SV(SV *point3_sv, Pointf3 *point)
{
    ...
    if (!looks_like_number(sv_x) || !looks_like_number(sv_y) || !looks_like_number(sv_z))
        return false;
...
{
    if (!sv_isa(surface_sv, perl_class_name(THIS))
        && !sv_isa(surface_sv, perl_class_name_ref(THIS)))
        CONFESS("Not a valid %s object", perl_class_name(THIS));
