...
SCENARIO("PrintObject: object layer heights")
{
    GIVEN("20mm cube and default initial config, initial layer height of 2mm")
    {
        ...
        WHEN(
            "generate_object_layers() is called for 10mm layer heights and nozzle diameter of 11mm")
        {
        ...
        WHEN(
            "generate_object_layers() is called for 15mm layer heights and nozzle diameter of 16mm")
        {
    ...
    GIVEN("20mm cube and default initial config, initial layer height of 0.1mm")
    {
        ...
        WHEN("min shell thickness is 1.22 with layer height of 0.1")
        {
            ...
            for (int i = 0; i < 20; i++)
                print->objects[0]->layers[i]->make_fills();
            AND_THEN(
                "Layers 0-12 are solid (bottom of layer >= 1.22) (all fill_surfaces are solid)")
            {
                for (int i = 0; i < 13; i++)
                {
                    CHECK(print->objects[0]->layers[i]->print_z <= (i + 1 * 0.1));
        ...
        WHEN("min shell thickness is 1.22 14 bottom layers")
        {
            ...
            for (int i = 0; i < 20; i++)
                print->objects[0]->layers[i]->make_fills();
            AND_THEN(
                "Layers 0-13 are solid (bottom of layer >= 1.22) (all fill_surfaces are solid)")
            {
                for (int i = 0; i < 14; i++)
                {
                    CHECK(print->objects[0]->layers[i]->print_z <= (i + 1 * 0.1));
