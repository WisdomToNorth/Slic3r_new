...
bool test_if_solid_surface_filled(const ExPolygon &expolygon, double flow_spacing, double angle = 0,
                                  double density = 1.0);
...
TEST_CASE("Fill: Pattern Path Length")
{
    ...
    SECTION("Square")
    {
        ...
        {
            std::transform(points.cbegin() + i, points.cend(), std::back_inserter(test_set),
                           [](const Pointf &a) -> Point { return Point::new_scale(a); });
            std::transform(points.cbegin(), points.cbegin() + i, std::back_inserter(test_set),
                           [](const Pointf &a) -> Point { return Point::new_scale(a); });
            Polylines paths{test(Slic3r::ExPolygon(test_set))};
            REQUIRE(paths.size() == 1); // one continuous path

            // TODO: determine what the "Expected length" should be for rectilinear fill of a
            // 100x100 polygon. This check only checks that it's above scale(3*100 + 2*50) +
            // scaled_epsilon. ok abs($paths->[0]->length - scale(3*100 + 2*50)) - scaled_epsilon,
            // 'path has expected length';
            REQUIRE(std::abs(paths[0].length() - static_cast<double>(scale_(3 * 100 + 2 * 50)))
                        - SCALED_EPSILON
                    > 0); // path has expected length
    ...
    {
        Pointfs points{Pointf(0, 0),     Pointf(100, 0), Pointf(150, 50),
                       Pointf(100, 100), Pointf(0, 100), Pointf(-50, 50)};
        Points test_set;
        test_set.reserve(6);
    ...
    SECTION("Square with hole")
    {
        ...
        std::transform(square.cbegin(), square.cend(), std::back_inserter(test_square),
                       [](const Pointf &a) -> Point { return Point::new_scale(a); });
        std::transform(hole.cbegin(), hole.cend(), std::back_inserter(test_hole),
                       [](const Pointf &a) -> Point { return Point::new_scale(a); });
    ...
    SECTION("Regression: Missing infill segments in some rare circumstances")
    {
        ...
        // TODO: determine what the "Expected length" should be for rectilinear fill of a 100x100
        // polygon. This check only checks that it's above scale(3*100 + 2*50) + scaled_epsilon. ok
        // abs($paths->[0]->length - scale(3*100 + 2*50)) - scaled_epsilon, 'path has expected
        // length';
        REQUIRE(std::abs(paths[0].length() - static_cast<double>(scale_(3 * 100 + 2 * 50)))
                    - SCALED_EPSILON
                > 0); // path has expected length
    ...
    {
        Points square{Point::new_scale(0, 0), Point::new_scale(50, 0), Point::new_scale(50, 50),
                      Point::new_scale(0, 50)};
    ...
    {
        Points points{
            Point::new_scale(6883102, 9598327.01296997), Point::new_scale(6883102, 20327272.01297),
            Point::new_scale(3116896, 20327272.01297), Point::new_scale(3116896, 9598327.01296997)};
    ...
    SECTION("Solid surface fill")
    {
        Points points{Point::new_scale(0, 0), Point::new_scale(98, 0), Point::new_scale(98, 10),
...

{
    ...
    is_deeply
        [ map $_->[Y], map @{$_->polyline},
@{$collection->chained_path_from(Slic3r::Point->new(0,30), 0)} ], [20, 18, 15, 10, 8, 5], 'chained
path';
}

{
    ...
    is_deeply
        [ map $_->[X], map @{$_->polyline},
@{$collection->chained_path_from(Slic3r::Point->new(30,0), 0)} ], [reverse 4, 10, 15, 10, 15, 20],
        'chained path';
}
...
for my $pattern (qw(rectilinear honeycomb hilbertcurve concentric)) {
    ...
    ok !(defined first { !$convex_hull->contains_point($_) } @infill_points), "infill does not
exceed perimeters ($pattern)";
}

{
    ...
    $config->set('cooling', 0);                 # for preventing speeds from being altered
        ...
        my $test = sub {
            ...
            return unscale unscale sum(map $_->area, @{offset([$convex_hull],
scale(+$config->infill_extrusion_width/2))});
        };
...

{
    ...
    ok sum(map unscale unscale $_->area*-1, @holes) < 1, 'no gaps between top solid infill and
perimeters';
}
*/

bool test_if_solid_surface_filled(const ExPolygon &expolygon, double flow_spacing, double angle,
                                  double density)
{
    auto *filler{Slic3r::Fill::new_from_type("rectilinear")};
    filler->bounding_box = expolygon.bounding_box();
