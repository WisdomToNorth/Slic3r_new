...
int get_brim_tool(std::stringstream &gcode, Slic3r::GCodeReader &parser)
{
    ...
    parser.parse_stream(
        gcode,
        [&tool, &brim_tool, &m, tool_regex](Slic3r::GCodeReader &self,
...
TEST_CASE("Skirt height is honored")
{
    ...
    parser.parse_stream(gcode,
                        [&layers_with_skirt, &support_speed](
    ...
    GIVEN("A default configuration")
    {
        ...
        WHEN("Brim width is set to 5")
        {
            ...
            THEN("Brim is generated")
            {
                ...
                auto support_speed =
                    config->get<ConfigOptionFloat>("support_material_speed") * MM_PER_MIN;
                parser.parse_stream(
                    gcode,
                    [&brim_generated, support_speed](Slic3r::GCodeReader &self,
                                                     const Slic3r::GCodeReader::GCodeLine &line)
                    {
        ...
        WHEN("Object is plated with overhang support and a brim")
        {
            ...
            {
                CHECK(config->getBool(
                    "support_material")); // test is not valid if support material is off
                ...
                parser.parse_stream(
                    gcode,
                    [tool_regex, &m, config, &extrusion_points, &tool, &skirt_length,
                                    ...
                                    const Slic3r::GCodeReader::GCodeLine &line)
                    {
                        ...
                        if (std::regex_match(line.cmd, m, tool_regex))
                        {
                            tool = std::stoi(m[1].str());
                                 ...
                                 == Approx(
                                     config->get<ConfigOptionFloat>("first_layer_height").value))
                        {
                            // on first layer
                            if (line.extruding() && line.dist_XY() > 0)
                            {
                                ...
                                if (speed == Approx(support_speed)
                                    && tool == config->getInt("perimeter_extruder") - 1)
                                {
                                ...
                                {
                                    extrusion_points.push_back(
                                        Slic3r::Point::new_scale(line.new_X(), line.new_Y()));
                                }
                            }
                        }
