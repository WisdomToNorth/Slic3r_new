...
SCENARIO("PrintGCode basic functionality")
{
    GIVEN("A default configuration and a print test object")
    {
        ...
        WHEN("the output is executed with no support material")
        {
            ...
            {
                REQUIRE(exported.find("; external perimeters extrusion width")
                        != std::string::npos);
            ...
            THEN(
                "Config options emitted for print config, default region config, default object config")
            {
            ...
            THEN("final Z height is ~20mm")
            {
                ...
                reader.parse(exported,
                             [&final_z](GCodeReader &self, const GCodeReader::GCodeLine &line)
                             {
                                 final_z = std::max(
                                     final_z, static_cast<double>(
                                                  self.Z)); // record the highest Z point we reach
                             });
        ...
        WHEN("output is executed with complete objects and two differently-sized meshes")
        {
            ...
            auto print{Slic3r::Test::init_print({TestMesh::cube_20x20x20, TestMesh::ipadstand},
                                                model, config)};
            Slic3r::Test::gcode(gcode, print);
            auto exported{gcode.str()};
            THEN("Some text output is generated.")
            ...
            THEN("Z height resets on object change")
            {
                ...
                reader.parse(
                    exported,
                    [&final_z, &reset](GCodeReader &self, const GCodeReader::GCodeLine &line)
                    {
                        if (final_z > 0 && std::abs(self.Z - 0.3) < 0.01)
                        { // saw higher Z before this, now it's lower
                            reset = true;
                        }
                        else
                        {
                            final_z = std::max(
                                final_z,
                                static_cast<double>(self.Z)); // record the highest Z point we reach
                        }
                    });
                REQUIRE(reset == true);
            }
        ...
        WHEN("layer_num represents the layer's index from z=0")
        {
            ...
            Slic3r::Model model;
            auto print{Slic3r::Test::init_print({TestMesh::cube_20x20x20, TestMesh::cube_20x20x20},
                                                model, config)};
            Slic3r::Test::gcode(gcode, print);
