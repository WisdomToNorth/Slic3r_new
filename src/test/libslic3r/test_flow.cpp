...
{
    GIVEN(
        "A config with a skirt, brim, some fill density, 3 perimeters, and 1 bottom solid layer and a 20mm cube mesh")
    {
        // this is a sharedptr
        ...
        WHEN("first layer width set to 2mm")
        {
            ...
            parser.parse_stream(
                ...
                {
                    if (self.Z == Approx(layer_height).margin(0.01))
                    { // only consider first layer
            ...
            {
                bool pass = false;
                auto avg_E{std::accumulate(E_per_mm_bottom.cbegin(), E_per_mm_bottom.cend(), 0.0)
                           / static_cast<double>(E_per_mm_bottom.size())};

                pass = (std::count_if(E_per_mm_bottom.cbegin(), E_per_mm_bottom.cend(),
                                      [avg_E](const double &v) { return v == Approx(avg_E); })
                        == 0);
                REQUIRE(pass == true);
                REQUIRE(E_per_mm_bottom.size()
                        > 0); // make sure it actually passed because of extrusion
            }
...
{
    GIVEN(
        "A default config with no cooling and a fixed bridge speed, flow ratio and an overhang mesh.")
    {
    ...
    GIVEN(
        "A default config with no cooling and a fixed bridge speed, flow ratio, fixed extrusion width of 0.4mm and an overhang mesh.")
    {
        WHEN("bridge_flow_ratio is set to 1.0")
        {
            THEN("Output flow is as expected.")
...
SCENARIO("Flow: Flow math for non-bridges", "[!mayfail]")
{
    GIVEN("Nozzle Diameter of 0.4, a desired width of 1mm and layer height of 0.5")
    {
        ...
        {
            auto flow{Flow::new_from_config_width(frExternalPerimeter,
                                                  ConfigOptionFloatOrPercent(0, false),
                                                  nozzle_diameter, layer_height, 0.0f)};
            REQUIRE(flow.spacing()
                    == Approx((1.1 * nozzle_diameter) - layer_height * (1.0 - PI / 4.0)));
        }

        THEN("Internal perimeter flow has spacing of 1.05 (minimum)")
        {
            auto flow{Flow::new_from_config_width(frPerimeter, ConfigOptionFloatOrPercent(0, false),
                                                  nozzle_diameter, layer_height, 0.0f)};
            REQUIRE(flow.spacing()
                    == Approx((1.05 * nozzle_diameter) - layer_height * (1.0 - PI / 4.0)));
        }
        THEN("Spacing for supplied width is 0.8927f")
        {
            auto flow{Flow::new_from_config_width(frExternalPerimeter, width, nozzle_diameter,
                                                  layer_height, 0.0f)};
    ...
    GIVEN("Nozzle Diameter of 0.25")
    {
        ...
        WHEN("layer height is set to 0.2")
        {
            ...
            {
                auto flow{Flow::new_from_config_width(frPerimeter,
                                                      ConfigOptionFloatOrPercent(0, false),
                                                      nozzle_diameter, layer_height, 0.0f)};
...
SCENARIO("Flow: Flow math for bridges", "[!mayfail]")
{
    GIVEN("Nozzle Diameter of 0.4, a desired width of 1mm and layer height of 0.5")
    {
        ...
        {
            auto flow{Flow::new_from_config_width(frInfill, width, nozzle_diameter, layer_height,
                                                  bridge_flow)};
            THEN("Bridge width is same as nozzle diameter")
            {
                REQUIRE(flow.width == Approx(nozzle_diameter));
        ...
        {
            auto flow{Flow::new_from_config_width(frPerimeter, width, nozzle_diameter, layer_height,
                                                  bridge_flow)};
            THEN("Bridge width is same as nozzle diameter")
            {
                REQUIRE(flow.width == Approx(nozzle_diameter));
        ...
        {
            auto flow{Flow::new_from_config_width(frSupportMaterial, width, nozzle_diameter,
                                                  layer_height, bridge_flow)};
            THEN("Bridge width is same as nozzle diameter")
            {
                REQUIRE(flow.width == Approx(nozzle_diameter));
