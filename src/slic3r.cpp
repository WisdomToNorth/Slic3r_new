...
int CLI::run(int argc, char **argv)
{
    ...
    for (auto const &file : config.getStrings("load", {}))
    {
        ...
        {
            Slic3r::Log::error("CLI")
                << "Error with the config file '" << file << "': " << e.what() << std::endl;
            exit(EXIT_FAILURE);
        }
    }
    ...
    for (auto const &opt_key : this->transforms)
    {
        if (opt_key == "merge")
        {
            ...
            if (!this->config.getBool("dont_arrange"))
            {
                ...
                m.arrange_objects(this->full_print_config.min_object_distance(),
        ...
        else if (opt_key == "duplicate")
        {
            ...
            for (auto &model : this->models)
            {
                const bool all_objects_have_instances =
                    std::none_of(model.objects.begin(), model.objects.end(),
                                 [](ModelObject *o) { return o->instances.empty(); });
                if (all_objects_have_instances)
                {
                    // if all input objects have defined position(s) apply duplication to the whole
                    // model
                    model.duplicate(this->config.getInt("duplicate"),
                                    this->full_print_config.min_object_distance(), &bb);
                }
                else
                {
                    model.add_default_instances();
                    model.duplicate_objects(this->config.getInt("duplicate"),
                                            this->full_print_config.min_object_distance(), &bb);
                }
            }
        }
        ...
        else if (opt_key == "scale_to_fit")
        {
            ...
            {
                Slic3r::Log::error("CLI")
                    << "--scale-to-fit requires a positive volume" << std::endl;
                exit(EXIT_FAILURE);
            }
            for (auto &model : this->models)
                for (auto &o : model.objects)
        ...
        else if (opt_key == "cut" || opt_key == "cut_x" || opt_key == "cut_y")
        {
            ...
            for (auto &model : this->models)
            {
                ...
                if (upper_obj->facets_count() > 0)
                    new_models.push_back(upper);
                if (lower_obj->facets_count() > 0)
                    new_models.push_back(lower);
            }
        ...
        else if (opt_key == "cut_grid")
        {
            ...
            for (auto &model : this->models)
            {
                ...
                TriangleMeshPtrs meshes =
                    mesh.cut_by_grid(config.opt<ConfigOptionPoint>("cut_grid")->value);
                size_t i = 0;
    ...
    for (auto const &opt_key : this->actions)
    {
        ...
        else if (opt_key == "export_gcode")
        {
            for (const Model &model : this->models)
            {
                ...
                print.center =
                    !this->config.has("center") && !this->config.has("align_xy") && print.arrange;
                Slic3r::Log::error("CLI")
                    << "Arrange: " << print.arrange << ", center: " << print.center << std::endl;
                ...
                boost::nowide::cout << std::fixed << std::setprecision(0) << "Done. Process took "
                                    << (duration / 60) << " minutes and " << std::setprecision(3)
                                    << std::fmod(duration, 60.0) << " seconds." << std::endl
                                    << std::setprecision(2)
                                    << "Filament required: " << print.total_used_filament() << "mm"
                                    << " (" << print.total_extruded_volume() / 1000 << "cm3)"
                                    << std::endl;
            }
        }
        else if (opt_key == "print")
        {
            ...
            {
                Slic3r::Log::error("CLI")
                    << "error: --print is not supported for multiple jobs" << std::endl;
                exit(EXIT_FAILURE);
            }
            ...
            {
                Slic3r::Log::error("CLI")
                    << "error: no G-code file to send; supply a model to slice or --gcode-file"
                    << std::endl;
                exit(EXIT_FAILURE);
            }
            ...
            {
                Slic3r::Log::error("CLI")
                    << "error: missing required --serial-port and --serial-speed" << std::endl;
                exit(EXIT_FAILURE);
            }

            // Connect to printer
            Slic3r::GCodeSender sender;
            sender.connect(this->print_config.getString("serial_port"),
        ...
        {
            Slic3r::Log::error("CLI")
                << "error: option not supported yet: " << opt_key << std::endl;
            exit(EXIT_FAILURE);
        }
    }
...
{
    boost::nowide::cout << "Slic3r " << SLIC3R_VERSION << " (build commit: " << BUILD_COMMIT << ")"
                        << std::endl
                        << "https://slic3r.org/ - https://github.com/slic3r/Slic3r" << std::endl
                        << std::endl
    ...
    {
        boost::nowide::cout << std::endl
                            << "Run --help-options to see the full listing of print/G-code options."
                            << std::endl;
    }
}
...
std::string CLI::output_filepath(const Model &model, IO::ExportFormat format) const
{
    ...
    std::string filename_format =
        this->print_config.getString("output_filename_format", "[input_filename_base]");
